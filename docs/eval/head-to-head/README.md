...
python3 docs/eval/head-to-head/run_matrix.py
```

Binaries default to `target/release/julie-server` and `~/source/miller/src/Miller.Server/bin/Release/net10.0/miller`. Override with `--julie-bin` and `--miller-bin`. Filter with `--repos` and `--tasks`. Skip a product with `--skip-julie` or `--skip-miller`.

The runner writes `results/<UTC timestamp>.json` and `results/<UTC timestamp>.md`. Run twice. Keep the second pair; the first run warms both indexes.
...
- Inspect targets are the first top-level symbol `julie-server symbols <expected path> --json` listed on that file. Search output is never used to pick a target.
