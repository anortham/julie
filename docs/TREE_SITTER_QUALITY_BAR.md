...
The 2026-05-07 validation pass restored the historical language verification docs, reconnected the current capability matrix to its required finding evidence, and added a generated current-HEAD certification report at [LANGUAGE_CERTIFICATION_REPORT.md](LANGUAGE_CERTIFICATION_REPORT.md). The certification report now consumes checked-in real-world smoke evidence from [LANGUAGE_REAL_WORLD_EVIDENCE.json](LANGUAGE_REAL_WORLD_EVIDENCE.json), generated by `cargo xtask certify tree-sitter --real-world --profile smoke`. That proves the current smoke corpus, not the whole restored historical matrix.

This still cannot be called achieved until the fixed-target release gates below pass at the current commit SHA, followed by live MCP dogfood after a release rebuild and restart. The historical plan evidence and restored real-world verification reports prove specific older commits and scopes. They do not prove this quality bar unless the commit SHA, scope label, and target requirements all match.
...
| Restored full real-world verification docs are historical, not current certification evidence | Open | Expand `fixtures/extraction/tree-sitter-real-world-corpus.toml` evidence beyond the smoke profile to cover the restored language matrix, then feed that evidence into the generated certification report. |
| VB.NET has current extractor support but no restored real-world verification row | Open | Add a VB.NET reference project and run the language verification checklist against the current implementation. |
| TSX and JSX have separate registry rows but are not split out in the restored manual verification matrix | Open | Keep TSX and JSX split out in the generated certification report and add dedicated real-world evidence if release claims mention variant-specific coverage. |
| Current capability matrix records 33 rows with open gap entries | Open | Close each gap with fixture evidence, implementation work, or a real exception. Do not treat an open gap as a release claim. |
