...
**Status:** Linux and local Windows package qualification, fresh install, compatible upgrade, semantics, repository gate, and real-agent evidence for Codex, OpenCode, and AGY pass. Current source candidate `1ff96903eed69d136e7a89a6dc44f2223fbc7be3` passed the frozen-tree full gate (5/5, 27.7s) before later qualification-only scripts/tests. The owner's `tag it and release it` command (2026-09-13) completes the local release-candidate native scope despite unavailable local macOS arm64/Intel prequalification. `.github/workflows/release.yml` still builds, packages, and verifies both actual macOS archives before its dependent GitHub-release job can create a release. Local macOS install/upgrade evidence is a post-release gap, not a pass. Claude Code, Hermes, and Cursor adoption/removal qualification remains pending post-release and does not block v8 or Plan 5C. Publication checks remain pending until the workflow succeeds.
...
| Windows upgrade guest source SHA | `012e6460083b312dc2c5efcc82cc00a2b68bdd58` |
...
| macOS arm64 | `julie-v8.0.0-aarch64-apple-darwin.tar.gz` | Not retained: upload skipped after later failure | Fresh profile: extract, `julie-server --version`, stdio initialize, second session, `service restart`, offline cached semantic check | POST-RELEASE GAP: the owner approved release despite unavailable local prequalification. Host key verification passed, but strict plain SSH and exact Tailscale ProxyCommand authentication return `Permission denied`; no SSH identity/access is available. Historical [run 34760490971](https://github.com/anortham/julie/actions/runs/34760490971) is historical only. The release workflow still builds, packages, and verifies the actual archive before GitHub release creation. |
...
- **OWNER-APPROVED RELEASE SCOPE — native qualification:** The `tag it and release it` command authorizes v8 release despite unavailable local macOS prequalification. Windows fresh install, lifecycle, and real v7.18.1-to-v8 migration pass locally. macOS local fresh-install and upgrade evidence is post-release work; [run 34760490971](https://github.com/anortham/julie/actions/runs/34760490971), at pushed ref `8c84e683`, remains historical only. This approval does not waive `.github/workflows/release.yml`: both macOS archives must build, package, and pass `verify-release-archive.py` before the dependent GitHub-release job can create a release.
- **PENDING — Plan 5C gate:** Do not start paired retrieval or agent-task experiments until the actual release workflow has passed its four native archive jobs and publication evidence is recorded.
