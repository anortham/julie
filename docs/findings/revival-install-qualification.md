...
| Latest qualification harness fix | `12a03cab` (native semantic tool parameter fix); verifier capture fix `13de611f`; portable version fix `ee3e86c9` |
...
| macOS arm64 | `julie-v8.0.0-aarch64-apple-darwin.tar.gz` | Not retained: upload skipped after later failure | Fresh profile: extract, `julie-server --version`, stdio initialize, second session, `service restart`, offline cached semantic check | INCOMPLETE: [run 34757538883](https://github.com/anortham/julie/actions/runs/34757538883) built and verified the archive, proving the portable-version fix, then cached offline semantics failed because required mode was supplied only to the CLI rather than the tool parameters. |
| macOS Intel | `julie-v8.0.0-x86_64-apple-darwin.tar.gz` | Not retained: upload skipped after later failure | Fresh profile: extract, `julie-server --version`, stdio initialize, second session, `service restart`, offline cached semantic check | INCOMPLETE: [run 34757538883](https://github.com/anortham/julie/actions/runs/34757538883) built and verified the archive, proving the portable-version fix, then cached offline semantics failed because required mode was supplied only to the CLI rather than the tool parameters. |
| Windows x64 | `julie-v8.0.0-x86_64-pc-windows-msvc.zip` | Not retained: upload skipped after later failure | `native-qualification.yml`: fresh profile, start old packaged `julie-server.exe`, extract candidate into a distinct version directory, retain lock evidence | INCOMPLETE: [run 34757538883](https://github.com/anortham/julie/actions/runs/34757538883) built and verified the archive, proving the file-backed verifier fix, then cached offline semantics failed because required mode was supplied only to the CLI rather than the tool parameters. Lock, lifecycle, and upload steps were skipped. `12a03cab` passes `semantics=required` to tool parameters with deterministic RED/GREEN coverage; native retry budget is exhausted and no third run was pushed. |
...
- **INCOMPLETE — native qualification:** [run 34757538883](https://github.com/anortham/julie/actions/runs/34757538883) built and verified all three archives, proving `ee3e86c9` portable version derivation and `13de611f` file-backed verifier capture. All three then failed cached offline semantics because required mode was supplied only to the CLI rather than tool parameters; Windows lock, lifecycle, and upload steps were skipped. `12a03cab` fixes the parameters with exact RED/GREEN coverage, but the broad native retry budget is exhausted and no third run was pushed.
