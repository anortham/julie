...
| Codex pre-merge review fixes worker invariants: (1) live legacy PID files are preserved across an upgrade, (2) `create_exclusive` bails when a live legacy daemon owns the file, (3) `WorkspacePool::shutdown` returns within 5s even when one workspace's `search_index` mutex is held by a hung task. Tests assert preservation, dead-cleanup, bail-with-live-pid, and bounded-shutdown timing | `cargo nextest run --lib test_check_running_preserves_legacy_pid_file_when_alive`, `test_check_running_removes_legacy_pid_file_when_dead`, `test_create_exclusive_bails_on_live_legacy_pid_file`, `test_workspace_pool_shutdown_bounded_when_mutex_is_held` | worker-red-green | 40f63442e7d34e8e3e0a37db13aaa9ce1f2a5b04 | pass (4 new tests + 26 daemon::pid tests overall) | 2026-05-07T00:18:00Z | no |
| Lead post-codex-fix branch gate: reliability tier (daemon + workspace_init + integration) passes against the final HEAD after codex review fixes land. Confirms the legacy-PID-handling change does not regress existing daemon liveness tests, and the bounded-shutdown change does not regress the LIFO ordering tests | `cargo xtask test reliability` | branch-gate | 40f63442e7d34e8e3e0a37db13aaa9ce1f2a5b04 | pass (3/3 buckets, 62.3s) | 2026-05-07T00:23:00Z | no |
