...
| `WorkspacePool::active_count` | `delete` | Removed method and rewrote fossil tests | Raw `rg` showed only pool map-size tests; product code uses `get` and runtime snapshots. | `cargo nextest run --lib tests::daemon::workspace_pool` and `cargo xtask test changed` passed |
| `WorkspacePool::new` watcher/embedding params | `delete` | Removed migration-only args and call-site boilerplate | Session lifecycle ownership lives in `WorkspaceSessionAttachment`, not constructor injection. | `cargo nextest run --lib tests::daemon::workspace_pool` and `cargo xtask test changed` passed |
| `WatcherPool::increment_ref` | `delete` | Removed public raw ref-count hook | `attach` owns watcher ref increments; direct calls were test fossils. | `cargo nextest run --lib tests::daemon::watcher_pool` and `cargo xtask test reliability` passed |
| `WatcherPool::decrement_ref` | `merge-into-caller` | Merged saturated decrement and grace startup into `detach` | `detach` was the only production caller; tests now use the public lifecycle. | `cargo nextest run --lib tests::daemon::watcher_pool` and `cargo xtask test reliability` passed |
...
| worker-red-green | WorkspacePool behavior still passes after removing constructor migration args and the test-only active-count method. | `cargo nextest run --lib tests::daemon::workspace_pool 2>&1 \| tail -30` | working-tree at `21aef93c` | PASS, 16 tests | 2026-05-04T00:33:00Z |
| worker-red-green | Lifecycle sharing still reuses the same workspace after replacing pool count assertions with cache identity assertions. | `cargo nextest run --lib test_workspace_pool_sharing_across_sessions 2>&1 \| tail -30` | working-tree at `21aef93c` | PASS, 1 test | 2026-05-04T00:33:00Z |
| affected-change | Changed-file gate passes after WorkspacePool constructor cleanup. | `cargo xtask test changed 2>&1 \| tail -40` | working-tree at `21aef93c` | PASS, 22 buckets in 379.1s | 2026-05-04T00:37:00Z |
| worker-red-green | WatcherPool public lifecycle still covers ref counts, grace deadlines, reaping, and provider update after raw ref helpers are removed. | `cargo nextest run --lib tests::daemon::watcher_pool 2>&1 \| tail -30` | working-tree at `777026dd` | PASS, 7 tests | 2026-05-04T00:41:00Z |
| worker-red-green | Session attachment still increments watcher refs after raw `increment_ref` is removed. | `cargo nextest run --lib test_session_attachment_increments_watcher_ref 2>&1 \| tail -25` | working-tree at `777026dd` | PASS, 1 test | 2026-05-04T00:41:00Z |
| worker-red-green | Multiple session attachments still share watcher refs and start grace on final disconnect. | `cargo nextest run --lib test_watcher_pool_reuses_session_refs_and_starts_grace_on_last_disconnect 2>&1 \| tail -25` | working-tree at `777026dd` | PASS, 1 test | 2026-05-04T00:41:00Z |
| affected-change | Reliability gate passes after WatcherPool raw ref-count helper cleanup. | `cargo xtask test reliability 2>&1 \| tail -60` | working-tree at `777026dd` | PASS, 3 buckets in 55.5s | 2026-05-04T00:43:00Z |
