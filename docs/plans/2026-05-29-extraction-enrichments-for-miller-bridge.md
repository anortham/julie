...
| **Extractor output drift** | `EXTRACTION_CONTRACT_VERSION: &str` | `crates/julie-extractors/src/lib.rs:121` (now `"2026-05-29.bridge-anchors-v2"`; was `"2026-05-11.body-span-v1"` at plan start) | string | The canonical extractor OUTPUT shape changes. **Drift detection is via `SEMANTIC_INDEX_ENGINE_VERSION`, NOT `capabilities.json`** — see below. |
