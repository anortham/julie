...
**Deliverable:** the two resolver-critical literal legs that are dropped today are captured as first-class records: **(a)** TS `fetch`/`axios.get('/api/users')` URL call-arguments, and **(b)** inline C# Dapper/ADO `Query<T>("... FROM Users ...")` SQL body literals. (C# route attrs like `[Route("/api/users")]` are already captured via `AnnotationMarker.raw_text` once Phase 1 wires class/method annotations — reuse/normalize, optionally re-emit as `kind:Route` for query uniformity; do not re-extract.)
...
**Minimal capture sites (exactly two legs — do not broaden the carrier match silently):**
1. **TS** — in the existing `call_expression` arm (`identifiers.rs:51-90`): after the callee, when the callee is in an HTTP carrier set `{fetch, $fetch, request, axios, axios.get/post/put/delete/patch}`, read `child_by_field_name("arguments")`, decode string/template children, emit `Literal{kind:Url, carrier:<callee>, arg_position}`. **Dotted callees need the object child:** for `axios.get('/url')` the `function` field is a `member_expression`; the existing arm (`identifiers.rs:68-83`) reads only the `property` child (`get`). To match `axios.get`, read `child_by_field_name("object")` (`axios`) and combine with the property before testing the carrier set. Add a test for both `fetch('/url')` and `axios.get('/url')`.
2. **C#** — in the existing `invocation_expression` arm (`identifiers.rs:36-64`): when the callee matches a Dapper/ADO carrier set `{Query, QueryAsync, QueryFirst*, QuerySingle*, Execute*, ExecuteScalar*}` (optionally gated on a `member_access` receiver), descend the sibling `argument_list`, decode string children, emit `Literal{kind:Sql, carrier:<callee>, arg_position}`. **Do NOT relax `is_csharp_type_usage_identifier`** — that boundary governs the separate `identifier` arm; literal capture is a parallel emit inside the `invocation_expression` arm and shares no code with it (verified).

**Write-path + cleanup (honor all four cross-cutting rules — Phase 2 step 0 already introduced `CanonicalWriteSet`):**
1. New `Literal` struct + `LiteralKind` in `base/types.rs`, exported in `lib.rs`; `pub literals: Vec<Literal>` on `BaseExtractor` (next to `identifiers`), threaded through `extract_canonical` + `ExtractionResults` + `registry::extract_csharp`/`extract_typescript` + `ExtractedBatch` (new `all_literals`).
2. Migration **028** (pure new-table, copy `migration_026`): bump `LATEST_SCHEMA_VERSION` 27→**28**; `migration_028_add_literals_table()` delegates to `create_literals_table()` in `schema.rs`, called from `initialize_schema()` after the symbols/identifiers tables. All DDL lives in `schema.rs` only.
3. `insert_literals_tx` in `src/database/bulk/literals.rs` (mirror `bulk/identifiers.rs`); add `literals: &'a [Literal]` to `CanonicalWriteSet` (one struct edit; call sites compile-fail until populated).
4. **Cleanup in all THREE sites (Rule 1):** `delete_file_rows_tx` → `DELETE FROM literals WHERE file_path = ?1;`; `delete_all_indexed_rows_tx` → add `"DELETE FROM literals"`; `delete_workspace_data` → add `tx.execute("DELETE FROM literals", [])?;`. All dependent-first.
5. **Counts (Rule 4):** add `literals` to `InsertCounts` and `ExternalExtractCounts` + the `count_table(&conn, "literals")?` call.
...
**Exit:** TS URL call-args and inline C# SQL literals are queryable structured records, decoded for interpolation/concatenation, classifiable at read time, cleaned on re-index, and surfaced in `extract info`.
