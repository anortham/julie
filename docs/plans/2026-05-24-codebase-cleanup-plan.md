...
- **Candidate 12:** remaining slices are analysis/local SQL fixture helpers and larger behavior-file splits after those helpers exist. A read-only explorer ranked the next safest slice as the `test_quality_tests` pipeline fixture-helper cluster around `test_pipeline_integration_updates_metadata` and `test_pipeline_integration_with_identifier_evidence`; keep raw SQL edge cases local and migrate only narrow setup clusters. `test_linkage_tests` first setup cluster is a second option. Do not consolidate daemon roots, adapter duplex tests, schema/migration raw SQL, or analytics SQL edge cases without per-cluster review.
