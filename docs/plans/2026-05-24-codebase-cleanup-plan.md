...
- **Candidate 11:** broad handler/workspace/search/watcher/workspace-pool splits still wait on stronger contracts and harnesses. The next daemon DB split is likely workspace registry storage, but it is shared daemon/workspace lifecycle work and should stay lead-owned or coupled-tier. Avoid moving `migrate_workspace_ids` until table-specific modules exist.
- **Candidate 12:** remaining slices are analysis/local SQL fixture helpers and larger behavior-file splits after those helpers exist. `test_quality_tests` still has raw SQL in no-body, preserve-existing-metadata, and fixture-not-applicable pipeline tests that were intentionally left outside the first pipeline setup slice. `test_linkage_tests` still has later raw SQL edge cases for uncovered symbols, test-to-test exclusion, caps, ambiguity guards, stale cleanup, filters, parent aggregation, and best confidence; migrate only one reviewed cluster at a time. Do not consolidate daemon roots, adapter duplex tests, schema/migration raw SQL, or analytics SQL edge cases without per-cluster review.
