...
- **Candidate 12:** targeted builder migrations continue, but explicit stay-raw categories are now verified: malformed row-mapping fixtures in `database_row_mapping.rs`; target-workspace identifier fixtures that pass `kind='import'` because `IdentifierKind` has no `Import` variant; schema/default tests such as `test_reference_score_defaults_to_zero`; assertion SQL, metadata-reset SQL, stale-relationship deletion SQL, schema/migration SQL, and analytics edge-case SQL. Next migration order from re-review: remaining `database.rs` raw setup only after per-cluster review; `hybrid_search_tests.rs` only after adding/verifying `SymbolBuilder::doc_comment` support or another exact doc-comment-preserving path.
