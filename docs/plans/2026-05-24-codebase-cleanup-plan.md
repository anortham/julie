...
- **Candidate 12:** targeted builder migrations continue, but explicit stay-raw categories are now verified: malformed row-mapping fixtures in `database_row_mapping.rs`; target-workspace identifier fixtures that pass `kind='import'` because `IdentifierKind` has no `Import` variant; schema/default tests such as `test_reference_score_defaults_to_zero`; assertion SQL, metadata-reset SQL, stale-relationship deletion SQL, schema/migration SQL, ignored concurrency stress SQL, and analytics edge-case SQL. The remaining `database.rs` raw setup inserts are now classified as intentional stay-raw clusters: legacy migration coverage, ignored raw-concurrency stress writers, reference-score default coverage, and import-kind identifier scoring. `hybrid_search_tests.rs` file/symbol setup inserts are now migrated; re-run the raw setup inventory across `src/tests/tools`, `src/tests/core`, and `src/tests/analysis` before selecting the next helper slice.
