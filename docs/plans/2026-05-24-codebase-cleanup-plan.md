...
- **Candidate 11:** broad handler/workspace/search/watcher/workspace-pool splits still wait on stronger contracts and harnesses. Next safe daemon DB slices are likely `tool_calls` and `codehealth`; avoid `migrate_workspace_ids` until table-specific modules exist.
- **Candidate 12:** remaining slices are analysis/local SQL fixture helpers and larger behavior-file splits after those helpers exist. `test_quality_tests` and `test_linkage_tests` still contain raw SQL edge cases and should be migrated only in narrow clusters. Do not consolidate daemon roots, adapter duplex tests, schema/migration raw SQL, or analytics SQL edge cases without per-cluster review.
