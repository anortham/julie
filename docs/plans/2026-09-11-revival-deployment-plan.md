...
**Status:** The frozen candidate code/full-gate SHA is `055645e02ebf261355c7db6659a9fe1b5159a791`; the latest qualification harness fix is `12a03cab`, after portable-version fix `ee3e86c9` and verifier capture fix `13de611f`. Linux qualification and Codex/OpenCode/AGY real-agent workflows pass; the final full gate passed 5/5 in 43.9s at the candidate code SHA. [Native retry 34757538883](https://github.com/anortham/julie/actions/runs/34757538883) verified all three archives but failed cached offline semantics because required mode was omitted from tool parameters; `12a03cab` fixes that exact harness defect, but retry budget is exhausted. Compatible upgrade, macOS/Windows, remaining client routes, and hosted native retry remain incomplete. Plan 5C is blocked. No publication has started or is authorized.
...
- [ ] All native jobs execute their actual packaged server and prove instruction delivery. [Hosted retry 34757538883](https://github.com/anortham/julie/actions/runs/34757538883) built and verified all three archives, proving `ee3e86c9` portable version derivation and `13de611f` file-backed verifier capture. All three then failed cached offline semantics because required mode was supplied only to the CLI, not tool parameters; Windows lock, lifecycle, and upload steps were skipped. `12a03cab` fixes the exact parameter defect with RED/GREEN coverage, but native retry budget is exhausted and no third run was pushed.
