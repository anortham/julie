...
**Status:** Historical full-gate evidence at `055645e02ebf261355c7db6659a9fe1b5159a791` remains the provenance for the unrebuild historical Linux archive. The current broad full gate was `1ff96903eed69d136e7a89a6dc44f2223fbc7be3` (5/5, 27.7s), before later qualification-only scripts/tests; a third full run exceeds the branch retry budget and needs owner decision. Local win-test Windows qualification now passes for release build, archive verification, persistent-service offline semantics, NTFS executable lock, and lifecycle/discovery cleanup. Full Windows remains reserved for final post-merge validation. macOS, client authorization, compatible upgrade, and publication remain incomplete; Plan 5C stays blocked. No GitHub Actions run or push followed the user's directive.
...
- [x] Native Windows held the old executable open while the new archive extracted successfully elsewhere. Local `win-test` qualification passed with archive SHA-256 `5510d7c1cdff9418260def83a4b071196bdaa2ed7a029cc3e89d02f7bf3f4bfe`; Windows full remains reserved for final post-merge validation.
...
- [ ] All native jobs execute their actual packaged server and prove instruction delivery. Local win-test passed Windows release build, archive identity/manifest/instructions verification, persistent-service cached offline semantics (`163804df`), NTFS executable lock (`a9f9c66c`), and file-backed two-session PID reuse/restart/exit/stop/discovery cleanup (`67a37cb3`) at clean guest SHA `46816cf3`; no GitHub Actions run or push followed. macOS native evidence remains incomplete. Full Windows is reserved for final post-merge validation.
- [ ] The parent awaits the exact reusable workflow job, propagates its failure and exposes its run evidence. Publication-only pending; not a local release-candidate blocker.
- [ ] Archives and generated plugin source contain no maintainer override symlinks; all four flat archive layouts remain valid. Publication-only pending; not a local release-candidate blocker.
...
Produce a qualification table for Linux x64, macOS arm64, macOS Intel and Windows x64. Use fresh user profiles/homes; built packages must not discover maintainer caches. Local `win-test` now runs Julie and passed the Windows release build, package verification, persistent-service cached offline semantics, NTFS executable lock, and lifecycle/discovery cleanup. Windows full remains reserved for final post-merge validation. Missing macOS/native-client access stays an explicit incomplete gate.
...
- [ ] All advertised native archives have fresh install and upgrade evidence. The real Windows lock case passes locally; macOS fresh install and upgrade evidence remain pending.
...
|---|---|---|---|---|---|---|
| Local Windows native package qualification | win-test focused Rust, release build, package, persistent-service semantics, NTFS lock, and file-backed verifier/lifecycle probes | live | guest `46816cf3`; fixes `163804df`, `a9f9c66c`, `67a37cb3` | PASS: archive SHA-256 `5510d7c1cdff9418260def83a4b071196bdaa2ed7a029cc3e89d02f7bf3f4bfe`; archive identity/manifest/instructions, semantics, lock, PID reuse/restart/exit, stop and discovery cleanup | 2026-09-13 | Logs `/home/murphy/.local/share/win-test/logs/20260913T154842Z-revival-deployment-3836626.log`, `/home/murphy/.local/share/win-test/logs/20260913T155624Z-revival-deployment-3840637.log`, `/home/murphy/.local/share/win-test/logs/20260913T160508Z-revival-deployment-3844864.log`, `/home/murphy/.local/share/win-test/logs/20260913T161420Z-revival-deployment-3852262.log`, `/home/murphy/.local/share/win-test/logs/20260913T161722Z-revival-deployment-3854455.log`, `/home/murphy/.local/share/win-test/logs/20260913T162854Z-revival-deployment-3861460.log`, `/home/murphy/.local/share/win-test/logs/20260913T162955Z-revival-deployment-3862001.log`; no GitHub Actions or push. |
| Current frozen-tree Plan 4 branch gate | `cargo xtask test full` | full | tested `1ff96903eed69d136e7a89a6dc44f2223fbc7be3` | PASS: 5/5 phases in 27.7s; one in-scope docs-contract failure fixed exactly, then the single retry passed | 2026-09-13 | Current branch is a qualification-only descendant; evidence-only changes preserve the required labels/values and cannot affect code/workflow commands. |
...
| Windows locked-executable extraction | Local `win-test` packaged release build, extraction, NTFS lock, and lifecycle verifier | live | guest `46816cf3`; fixes `163804df`, `a9f9c66c`, `67a37cb3` | PASS: archive SHA-256 `5510d7c1cdff9418260def83a4b071196bdaa2ed7a029cc3e89d02f7bf3f4bfe` | 2026-09-13 | Local archive verification, persistent-service semantics, lock, PID reuse/restart/exit, stop, and discovery cleanup passed; Windows full remains reserved for final post-merge validation. |
| Nonpublishing Windows qualification workflow contract | `cargo nextest run -p xtask native_qualification_workflow_is_nonpublishing_and_exercises_windows_lock` | worker-red-green | `87f333dd` | PASS: workflow contract; real native Windows execution later passed locally via `win-test` | 2026-09-12T23:48Z | The contract requires checksum-bound archive verification, a fresh profile, running an old packaged shim during separate-version extraction, cleanup, and attributable candidate/archive hashes; the later local execution evidence is recorded above. |
