...
**Status:** Historical full-gate evidence at `055645e02ebf261355c7db6659a9fe1b5159a791` remains the provenance for the unrebuild historical Linux archive. Current source candidate `1ff96903eed69d136e7a89a6dc44f2223fbc7be3` passed `cargo xtask test full` 5/5 in 27.7s after one in-scope docs-contract failure was fixed exactly and its single retry passed. Evidence-only documentation changes may advance the branch after this tested SHA without affecting code/workflow commands. [Run 34760490971](https://github.com/anortham/julie/actions/runs/34760490971), at pushed qualification ref `8c84e683`, still leaves native qualification incomplete; Plan 5C remains blocked pending an explicitly authorized native run. No publication has started or is authorized.
...
- [ ] All native jobs execute their actual packaged server and prove instruction delivery. [Run 34760490971](https://github.com/anortham/julie/actions/runs/34760490971), at pushed ref `8c84e683`, built and archive-verified all three targets; macOS search returned `mode=auto`, `backend=lexical`, and `backend_fallback=true`, while Windows online model preparation failed immediately with Hugging Face HTTP 429 before search. Lifecycle, Windows lock, and upload steps were skipped. `ce574773` adds `python3 .github/scripts/qualify-native-offline-semantics.py --self-test` RED/GREEN for top-level `--semantics required` and fallback rejection. `3115c6c8` adds `cargo nextest run --lib cli_json_reply_preserves_required_full_readiness` RED/GREEN; `cargo check --bin julie-server` passes. A new frozen-tree full gate and authorized native run are required; no further run is authorized.
...
|---|---|---|---|---|---|---|
| Current frozen-tree Plan 4 branch gate | `cargo xtask test full` | full | tested/current `1ff96903eed69d136e7a89a6dc44f2223fbc7be3` | PASS: 5/5 phases in 27.7s; one in-scope docs-contract failure fixed exactly, then the single retry passed | 2026-09-13 | Any following evidence-only docs commit preserves the required labels/values and cannot affect code/workflow commands. |
