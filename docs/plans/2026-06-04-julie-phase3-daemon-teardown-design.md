...
## 11. Dashboard live-signal persist-vs-drop classification (3c prerequisite for G1/Option B)

**Context:** §8 decided Option B — standalone `registry.db` reader. Before 3d builds that reader, every live signal `DashboardState` consumes today must be classified: either **PERSIST** (written to `registry.db` so the reader can surface it) or **DROP** (ephemeral; the standalone reader omits or derives it another way). This is producer-side classification only — no standalone server is implemented here.

**Sources verified:** `src/dashboard/state.rs` (`DashboardState` struct :129, `health_snapshot` :277), `src/daemon/app.rs` (`serve` :226 — construction site), `src/daemon/database/workspaces.rs` (`WorkspaceRow` + write methods), `src/daemon/database/tool_calls.rs`, `src/daemon/shutdown.rs` (`RecoveryMarker` :63), `src/daemon/session.rs` (`SessionTracker` :92, `SessionPhaseCounts` :53), `crates/julie-core/src/indexing_state.rs` (`IndexingRuntimeSnapshot`).

### Persist-vs-drop table

| Signal | Source field / type | Classification | Rationale + write-site (for PERSIST) |
|--------|-------------------|---------------|--------------------------------------|
| **Workspace rows** (path, status, last\_indexed, last\_index\_duration\_ms) | `DashboardState::workspace_pool` → `WorkspaceRow` | **PERSIST** | Already in `daemon.db.workspaces`; maps 1-for-1 to `registry.db.workspaces`. Written by workspace\_pool on register / index completion / refresh via `DaemonDatabase::upsert_workspace` + `update_workspace_status`. |
| **Per-workspace symbol\_count / file\_count** | `WorkspaceRow.symbol_count`, `.file_count` | **PERSIST** | Already in `daemon.db.workspaces.symbol_count/file_count` via `update_workspace_stats()`. Same row, no new write-site. |
| **Per-workspace embedding\_model / vector\_count** | `WorkspaceRow.embedding_model`, `.vector_count` | **PERSIST** | Already in `daemon.db.workspaces`. Written by `update_embedding_model` + `update_vector_count`. Same row. |
| **Per-workspace session\_count** | `WorkspaceRow.session_count` | **PERSIST** | Already in `daemon.db.workspaces.session_count` via `increment/decrement_session_count()` on session connect/disconnect. Standalone reader shows last-known open session count (stale if leader crashed — acceptable). |
| **Tool-call history** | `DashboardState::daemon_db` → `tool_calls` table | **PERSIST** | Already in `daemon.db.tool_calls` via `DaemonDatabase::insert_tool_call` / `insert_tool_call_with_input_bytes`. Maps to `registry.db.tool_calls`. Retention via existing `prune_tool_calls(days)`. |
| **Search-compare runs** | `daemon.db.search_compare_runs` | **PERSIST (conditional)** | Already persisted. Maps to `registry.db.search_compare_runs`. **Note:** §7 deletion DAG schedules this table for removal with the dual-write cleanup (G7). If dropped in 3d, remove from dashboard simultaneously. |
| **RecoveryMarker** | `DashboardState::recovery_markers` (`Arc<Vec<RecoveryMarker>>`) | **PERSIST (durable file — no change needed)** | Already written to disk as JSON files under `~/.julie/recovery-*.json` by `append_recovery_marker()` at shutdown. Standalone reader calls `read_recovery_markers(paths)` on the same files. No DB table or new write-site required. |
| **ProjectionFreshness / SearchProjectionHealth** | Computed in `search_projection_health()` from `canonical_revisions` + `projection_states` in `symbols.db` | **PERSIST (already in symbols.db — no change needed)** | These tables are already WAL-durable per-workspace. Standalone reader opens each workspace's `symbols.db` in WAL read-only mode and reads `projection_states` directly. Write-site: indexing engine on each index cycle (unchanged). |
| **Daemon start\_time → uptime** | `DashboardState::start_time: Instant` | **PERSIST (new column needed)** | `Instant` is not serialisable across processes. Add `started_at_unix: i64` to `registry.db` (suggest: a `daemon_state` table with a single-row upsert). Leader writes it at startup; standalone reader computes `uptime = now - started_at_unix`. Without this, the standalone dashboard shows no uptime. Write-site: leader process in `DaemonApp::new` or early in `serve`. |
| **Live active session count** | `SessionTracker::active_count()` | **DROP** | Purely in-process ephemeral state. The count is only meaningful inside the leader process. Per-workspace `session_count` in `workspaces` (PERSIST above) is the reader-accessible proxy. Live cross-workspace total is not separately persisted. |
| **Live per-session phase** (Connecting / Bound / Serving / Closing) | `SessionTracker::phase_counts()` → `SessionPhaseCounts` | **DROP** | Ephemeral lifecycle state inside the leader. Granular session phases (which session is in which state) are meaningless to a process that does not own those sessions. No useful proxy in `registry.db`. |
| **Live LifecyclePhase / ShutdownCause** | `DashboardState::daemon_phase: Arc<RwLock<LifecyclePhase>>` | **DROP** | In-process state machine. Fine-grained phase transitions (Starting → Running → Draining → Stopped) are only observable from inside the leader. A standalone reader infers "leader alive" from the freshness of `registry.db` rows (updated\_at timestamps). |
| **restart\_pending flag** | `DashboardState::restart_pending: Arc<AtomicBool>` | **DROP** | In-process signal set by `DaemonLifecycleController`. Meaningless after the leader exits; a new leader starts clean. No reader-side value. |
| **Live indexing activity** (active\_operation, stage, catchup\_active, watcher\_paused, watcher\_rescan\_pending, dirty\_projection\_count, repair\_needed, repair\_issue\_count, repair\_reasons) | `DashboardIndexingHealth` ← `WatcherPool` / `WorkspacePool` / `IndexingRuntimeSnapshot` | **DROP** | All are live in-process state from the watcher and workspace pools. A standalone reader is not running the watcher and cannot observe these signals. `dirty_projection_count` is partially derivable from `projection_states` in `symbols.db` (PERSIST); the rest (active operation name, watcher paused/rescan, repair scan details) are ephemeral and dropped. |
| **Live EmbeddingService state** (embedding\_available, embedding\_initializing, backend, accelerated, degraded\_reason) | `DashboardState::embedding_service: Option<Arc<EmbeddingService>>` → `DashboardRuntimePlaneHealth` | **DROP** | Live connection to the in-process or host-sidecar embedding service. A standalone dashboard reader does not own the embedding host (Phase 3b made it a separate process). The persisted `embedding_model` per workspace (PERSIST above) is the durable proxy. Live accelerator/degradation status is omitted. |
| **SSE broadcast channel** | `DashboardState::tx: broadcast::Sender<DashboardEvent>` | **DROP** | In-process fanout (ToolCall / SessionChange events). Cannot be bridged to a standalone reader without IPC. The standalone dashboard polls `registry.db` for fresh data instead of receiving push events. |
| **ErrorBuffer** (in-memory tracing ring) | `DashboardState::error_buffer: ErrorBuffer` | **DROP** | Ephemeral in-memory ring buffer populated by a `tracing_subscriber::Layer`. Not persisted anywhere today. Persisting to `registry.db.error_log` would add write overhead on every WARN/ERROR log line; the standalone reader can tail `~/.julie/daemon.log.*` directly. Dropped from the standalone dashboard. |
| **action\_csrf\_token** | `DashboardState::action_csrf_token: Arc<String>` | **DROP** | Dashboard mutation security token. Mutation routes (register / open / refresh / delete workspace, search\_compare) all require write access to the live in-process leader. The standalone reader dashboard is read-only; mutation routes are dropped. |
| **Mutation routes** (register, open, refresh, delete workspace; search\_compare) | Dashboard router action handlers | **DROP** | All require write access to the live system. The standalone reader has no connection to the leader's workspace\_pool / watcher\_pool. These are dropped from the Phase 3d dashboard; the CLI (`julie-server manage_workspace`) becomes the mutation surface. |
| **daemon\_db\_connected / workspace\_pool\_connected flags** | Computed in `health_snapshot` from `Option<Arc<DaemonDatabase>>` / `Option<Arc<WorkspacePool>>` | **DROP** | In-process plumbing health. A standalone reader infers "registry accessible" from whether it can open `registry.db`; it does not model the leader's internal connectivity. |

### Summary counts

- **PERSIST (already durable, no code change):** workspace rows + stats + session\_count, tool\_calls, search\_compare\_runs (until §7), RecoveryMarker, ProjectionFreshness.
- **PERSIST (new write-site needed):** `started_at_unix` → `registry.db.daemon_state` (single row, written by leader at startup).
- **DROP:** live session count/phases, LifecyclePhase/ShutdownCause/restart\_pending, live indexing activity (WatcherPool snapshot), live EmbeddingService state, SSE channel, ErrorBuffer, CSRF token, all mutation routes, in-process plumbing flags.

### Implications for 3d

1. The standalone reader needs only `registry.db` (workspace rows, tool\_calls, search\_compare) + per-workspace `symbols.db` (projection freshness) + `~/.julie/recovery-*.json` files (RecoveryMarker). No live process connections required.
2. The one new schema item is a `daemon_state` table (or row) in `registry.db` holding `started_at_unix`. This is the only schema addition this classification requires.
3. The standalone dashboard loses: live session list, live indexing progress, live embedding health, SSE push, error log, and all mutation actions. These are deliberately dropped — the Option B decision accepted this tradeoff.
4. `search_compare` dashboard page survives into the standalone reader (data is already persisted) unless §7 G7 cleanup removes the table first; coordinate the deletion.
