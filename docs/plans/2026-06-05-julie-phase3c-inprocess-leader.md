...
- **D7 — Index/lock location: ONE canonical per-workspace path, deterministic via `generate_workspace_id`.** T1 resolves project-local `.julie/indexes` vs shared `~/.julie/indexes` by reading the current standalone path resolution so the leader lock and the opened index always agree on the same inode (medium-risk mismatch the synthesis flagged). Documented in T1's output.
...
**Acceptance:** Integration test (build on `src/tests/harness/in_process.rs`) serves a full MCP session over stdio against a real workspace: initialize + on_initialized auto-index + a `fast_search` succeed end-to-end with NO daemon process; cwd-source deferral triggers. `cargo nextest run -p julie <serve in_process test>`.
...
4. **[high] The known fast_search/deep_dive hang resurfaces** — §5b per-request deadline is genuinely new; in-process there's no transport loop to host a deadline. *Mitigation:* T3's handler-level timeout + stalling-tool test; scope the deadline to read/query tools so it never aborts an in-flight write; the T6 host cold-spawn (≤180s) must not block startup (degrade lazily).
...
6. **[medium] Per-workspace lock-path inode mismatch** — stdio-layout vs daemon-layout could make two processes lock different files. *Mitigation:* T1 standardizes ONE canonical location (D7) + T8 derives `workspace_id` via the same `generate_workspace_id`; add a test that two processes with the same workspace path contend on the same inode.
