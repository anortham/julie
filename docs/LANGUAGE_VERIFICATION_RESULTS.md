...
This file is restored historical verification evidence, not a current release certification report.

Validated against the current implementation on 2026-05-07:

- Current machine-checked capability contract: `fixtures/extraction/capabilities.json`
- Generated current-HEAD report: [LANGUAGE_CERTIFICATION_REPORT.md](LANGUAGE_CERTIFICATION_REPORT.md)
- Generated current-HEAD real-world smoke evidence: [LANGUAGE_REAL_WORLD_EVIDENCE.json](LANGUAGE_REAL_WORLD_EVIDENCE.json)
- Focused contract gate: `cargo nextest run -p julie-extractors capability_matrix`
- Result after restoring required `docs/findings/` evidence files: PASS, 9/9 capability matrix tests
- Current registry rows: 36
- Historical matrix rows here: 33 user-facing language rows
- Missing from this historical matrix: VB.NET
- Current registry-only variant rows not tracked separately here: TSX and JSX, treated historically as TypeScript and JavaScript coverage
- Current capability gaps recorded in `capabilities.json`: 33 rows have at least one open gap entry, mostly pending-relationship fixture evidence gaps; `tsx`, `jsx`, and `vue` currently have no recorded gap entries

Read the PASS/FAIL/PARTIAL values below as historical real-world verification notes. A row is current only after fresh verification at the current commit SHA or [LANGUAGE_CERTIFICATION_REPORT.md](LANGUAGE_CERTIFICATION_REPORT.md) says so. The per-language files in `docs/verification/` are raw historical notes and may contain older FAIL/PARTIAL observations that were later fixed in this summary file.
