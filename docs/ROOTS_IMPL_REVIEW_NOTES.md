...
### Finding #1 ✅ FIXED — Version gate now rejects mismatched session when sessions > 0 AND the adapter waits for drain

Implemented across two commits:
- **Commit 1 (`dc6f623f`):** Refactored the gate into a pure `evaluate_version_gate(adapter_version, daemon_version, active_sessions) -> VersionGateOutcome` function in `src/daemon/ipc_session.rs`, with regression tests covering matching versions, missing version header (legacy adapter), no-active-sessions shutdown, active-sessions reject, and bidirectional mismatch. The accept loop in `src/daemon/mod.rs` now drops the stream and `continue`s on the reject path instead of falling through.
- **Commit 5 (Codex follow-up):** Codex's adversarial review caught that merely dropping the stream left the adapter's 3-retry budget (2 s × 3 = ~6 s) to fail against a daemon still advertising `ready`. Added `flag_restart_pending_after_version_reject()` which transitions `daemon_state` to `"stopping"` on the first rejection. The adapter's `ensure_daemon_ready` already handles `Stopping` by calling `wait_for_pid_exit` (60 s deadline) and respawning — so now new adapters wait for the old daemon to drain its active sessions instead of bouncing off in 6 s. Two unit tests cover the first-rejection-writes-stopping path and the idempotent-on-subsequent-calls property.
...
Implemented across two commits:
- **Commit 2 (`12877744`):** `list` and `remove` switched from `require_primary_workspace_identity()` to `current_workspace_id()` (Option), with sensible fallbacks — `list` shows registered workspaces with no `CURRENT`/`PAIRED` labels, `remove` skips the pairing-cleanup step when no primary is bound. `add` keeps the hard fail but with a new error pointing at `manage_workspace(operation="open", path=...)` or client roots instead of the irrelevant `index` operation. Three regression tests in `global_targeting.rs` cover the three tool-body surfaces via direct `ManageWorkspaceTool::call_tool` invocation.
- **Commit 5 (Codex follow-up):** Codex caught that on the real MCP path (`ServerHandler::call_tool`), the request-time preflight (`ensure_primary_workspace_for_request`) ran BEFORE the tool body because `manage_workspace_request_targets_primary` classified `"add" | "list" | "remove"` as primary-targeting. In a deferred Cwd session the preflight then silently bound the startup-hint workspace as primary, so `add` proceeded to pair the new reference against CWD instead of hard-failing with the new actionable error. Removed `"add"` from the primary-targeting classification in `src/handler.rs` so the tool body's hard-fail actually reaches real users. `list` / `remove` stayed in the classification intentionally — their tool-body tolerance is defense-in-depth, and keeping them in preflight preserves 25+ existing roots-resolution tests that use them as primary-scoped triggers. New regression test `test_manage_workspace_add_in_deferred_cwd_session_via_server_handler_rejects_without_primary_binding` exercises the real path end-to-end via `serve_directly`.
...
3. **Commit 5 (Codex follow-up):** Codex pointed out the `.julieignore` patch was only one instance of a broader asymmetry — the indexer's `should_index_file` accepts any extensionless text file (`Dockerfile`, `Makefile`, `Containerfile`, `.bashrc`, etc.) via `is_likely_text_file`, but `scan_workspace_files` rejected them all because `is_code_file` required a matching extension. Every repo with a Dockerfile got a phantom "deleted file" signal on reconnect. Extracted `is_likely_text_file` into `src/utils/mod.rs::file_utils` as a shared function, and rewrote `is_code_file` to use it for extensionless files plus apply `BLACKLISTED_FILENAMES` by name. Discovery's private method now delegates to the same free function so both sides stay in lockstep. New regression test `test_fresh_index_with_extensionless_text_files_needs_no_reindex` seeds a workspace with `Dockerfile` + `Makefile` + a `.rs` file and asserts no re-index is needed after a fresh index.
