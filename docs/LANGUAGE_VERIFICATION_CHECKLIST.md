...
This checklist is a restored historical verification method. It is still useful, but it is not the current source of truth by itself.

As of 2026-05-07, the current implementation contract is `fixtures/extraction/capabilities.json`, checked by `cargo nextest run -p julie-extractors capability_matrix` and summarized by the generated [LANGUAGE_CERTIFICATION_REPORT.md](LANGUAGE_CERTIFICATION_REPORT.md). That gate currently tracks 36 registry rows: the 34 user-facing languages plus TSX and JSX variant rows. The restored checklist originally omitted VB.NET and did not list TSX/JSX separately.

Use this checklist for manual real-world validation. Use the generated capability matrix and [LANGUAGE_CERTIFICATION_REPORT.md](LANGUAGE_CERTIFICATION_REPORT.md) for release-claim evidence.
...
| Zig | ZLS | |
| Lua | Lite | |
| VB.NET | dotnet/runtime or Roslyn Visual Basic samples | |
| Vue | LabHandbookV2 | |
...
TSX and JSX are tracked as separate registry rows in `capabilities.json`, but this manual checklist treats them as TypeScript and JavaScript variant coverage. The generated [LANGUAGE_CERTIFICATION_REPORT.md](LANGUAGE_CERTIFICATION_REPORT.md) splits them out explicitly.
