...
use crate::extractors::base::{BaseExtractor, PendingRelationship, Relationship, Symbol};
        ...
        if node.kind() == "call_expression" {
            if let Some(function_node) = node.child_by_field_name("function") {
                ...
                match symbol_map.get(function_name.as_str()) {
                    Some(called_symbol) if called_symbol.kind == crate::extractors::base::SymbolKind::Import => {
