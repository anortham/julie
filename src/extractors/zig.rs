...
impl ZigExtractor {
    ...
    fn extract_symbol_from_node(
        ...
        match node.kind() {
            ...
            "field_declaration" | "struct_field" | "container_field" => {
                self.extract_struct_field(node, parent_id)
            }
    ...
    fn extract_variable(&mut self, node: Node, parent_id: Option<&String>) -> Option<Symbol> {
        ...
        let name = self.base.get_node_text(&name_node);
        let is_const =
            node.kind() == "const_declaration" || self.base.get_node_text(&node).contains("const");
        let is_public = self.is_public_declaration(node);
        ...
        if let Some(_enum_node) = enum_node {
            let enum_match = regex::Regex::new(r"enum\(([^)]+)\)")
                .unwrap()
                .find(&node_text);
        ...
        if node_text.contains("error{") || node_text.contains("error {") {
            ...
            if node_text.contains("||") {
                let union_match = regex::Regex::new(r"error\s*\{[^}]*\}\s*\|\|\s*(\w+)")
                    .unwrap()
                    .captures(&node_text);
        ...
        if node_text.contains("fn (") || node_text.contains("fn(") {
            let fn_type_match = regex::Regex::new(r"=\s*(fn\s*\([^}]*\).*?)(?:;|$)")
                .unwrap()
                .captures(&node_text);
        ...
        if var_type == "inferred" && is_const {
            let assignment_match = regex::Regex::new(r"=\s*([^;]+)")
                .unwrap()
                .captures(&node_text);
    ...
    fn extract_from_error_node(
        ...
        let partial_match = regex::Regex::new(r"^const\s+(\w+)\s*\($")
            .unwrap()
            .captures(&node_text);
    ...
    fn traverse_for_relationships(
        ...
        match node.kind() {
            ...
            "const_declaration" => {
                ...
                if let Some(_struct_node) =
                    self.base.find_child_by_type(&node, "struct_declaration")
                {
                    self.extract_struct_relationships(node, symbols, relationships);
                }
            }
        ...
        for field_node in node.children(&mut cursor) {
            if field_node.kind() == "container_field" {
                if let Some(field_name_node) =
                    self.base.find_child_by_type(&field_node, "identifier")
                {
    ...
    fn extract_function_call_relationships(
            ...
            self.base.find_child_by_type(&node, "field_expression")
        {
            ...
            if identifiers.len() >= 2 {
                called_func_name = Some(self.base.get_node_text(&identifiers[1]));
                ...
                while let Some(parent) = current {
                    if matches!(
                        ...
                        if let Some(caller_name_node) =
                            self.base.find_child_by_type(&parent, "identifier")
                        {
    ...
    fn extract_identifier_from_node(&mut self, node: Node, symbol_map: &HashMap<String, &Symbol>) {
        match node.kind() {
            ...
            "call_expression" => {
                ...
                if let Some(name_node) = self.base.find_child_by_type(&node, "identifier") {
                    let name = self.base.get_node_text(&name_node);
