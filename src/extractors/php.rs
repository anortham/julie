...
impl PhpExtractor {
    ...
    fn visit_node(&mut self, node: Node, symbols: &mut Vec<Symbol>, parent_id: Option<String>) {
        ...
        let symbol = match node.kind() {
            ...
            "function_definition" | "method_declaration" => Some(self.extract_function(node, parent_id.as_deref())),
            ...
            "use_declaration" | "namespace_use_declaration" => Some(self.extract_use(node, parent_id.as_deref())),
    ...
    fn extract_class(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let name = self.find_child_text(&node, "name").unwrap_or_else(|| "UnknownClass".to_string());
        ...
        if let Some(extends_node) = extends_node {
            let base_class = self.base.get_node_text(&extends_node).replace("extends", "").trim().to_string();
            signature.push_str(&format!(" extends {}", base_class));
        }

        if let Some(implements_node) = implements_node {
            let interfaces = self.base.get_node_text(&implements_node).replace("implements", "").trim().to_string();
        ...
        metadata.insert("modifiers".to_string(), serde_json::Value::Array(
            modifiers.iter().map(|m| serde_json::Value::String(m.clone())).collect()
        ));

        if let Some(extends_node) = extends_node {
            metadata.insert("extends".to_string(), serde_json::Value::String(self.base.get_node_text(&extends_node)));
        }

        if let Some(implements_node) = implements_node {
            metadata.insert("implements".to_string(), serde_json::Value::String(self.base.get_node_text(&implements_node)));
        }
    ...
    fn extract_interface(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let name = self.find_child_text(&node, "name").unwrap_or_else(|| "UnknownInterface".to_string());

        let extends_node = self.find_child(&node, "base_clause");
        let mut signature = format!("interface {}", name);

        if let Some(extends_node) = extends_node {
            let base_interfaces = self.base.get_node_text(&extends_node).replace("extends", "").trim().to_string();
            signature.push_str(&format!(" extends {}", base_interfaces));
        }

        let mut metadata = HashMap::new();
        metadata.insert("type".to_string(), serde_json::Value::String("interface".to_string()));
        if let Some(extends_node) = extends_node {
            metadata.insert("extends".to_string(), serde_json::Value::String(self.base.get_node_text(&extends_node)));
        }
    ...
    fn extract_trait(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let name = self.find_child_text(&node, "name").unwrap_or_else(|| "UnknownTrait".to_string());

        let mut metadata = HashMap::new();
        metadata.insert("type".to_string(), serde_json::Value::String("trait".to_string()));
    ...
    fn extract_function(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let name = self.find_child_text(&node, "name").unwrap_or_else(|| "unknownFunction".to_string());
        ...
        let ref_prefix = if reference_modifier.is_some() { "&" } else { "" };
        ...
        if !modifiers.is_empty() {
            signature = signature.replace(&format!("function {}{}", ref_prefix, name),
        ...
        if let Some(params_node) = parameters_node {
            metadata.insert("parameters".to_string(), self.base.get_node_text(&params_node));
        } else {
            metadata.insert("parameters".to_string(), "()".to_string());
        }

        if let Some(return_node) = return_type_node {
            metadata.insert("returnType".to_string(), self.base.get_node_text(&return_node));
        }
        ...
        self.base.create_symbol(
            ...
            SymbolOptions {
                ...
                visibility: Some(self.determine_visibility(&modifiers)),
                parent_id: parent_id.map(|s| s.to_string()),
                metadata: Some(metadata.into_iter().map(|(k, v)| (k, serde_json::Value::String(v))).collect()),
                doc_comment: None,
            },
        )
    }
    ...
    fn find_return_type<'a>(&self, node: &Node<'a>) -> Option<Node<'a>> {
        ...
        for child in node.children(&mut cursor) {
            if found_colon {
                match child.kind() {
                    "primitive_type" | "named_type" | "union_type" | "optional_type" => return Some(child),
                    _ => {},
                }
            }
            if child.kind() == ":" {
    ...
    fn extract_property(&mut self, node: Node, parent_id: Option<&str>) -> Option<Symbol> {
        ...
        if let Some(type_node) = type_node {
            metadata.insert("propertyType".to_string(), self.base.get_node_text(&type_node));
        }
    ...
    fn extract_constant(&mut self, node: Node, parent_id: Option<&str>) -> Option<Symbol> {
        ...
        let mut signature = format!("{} const {}",
            match visibility {
    ...
    fn extract_namespace(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let name = self.find_child_text(&node, "namespace_name").unwrap_or_else(|| "UnknownNamespace".to_string());
    ...
    fn extract_use(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let (name, alias) = match node.kind() {
            "namespace_use_declaration" => {
                ...
                if let Some(use_clause) = self.find_child(&node, "namespace_use_clause") {
                    if let Some(qualified_name) = self.find_child(&use_clause, "qualified_name") {
                        ...
                        let alias = self.find_child(&node, "namespace_aliasing_clause")
            ...
            _ => {
                ...
                let name = self.find_child_text(&node, "namespace_name")
    ...
    fn extract_enum(&mut self, node: Node, parent_id: Option<&str>) -> Symbol {
        let name = self.find_child_text(&node, "name").unwrap_or_else(|| "UnknownEnum".to_string());
        ...
        if let Some(implements_node) = implements_node {
            let implements_clause = self.base.get_node_text(&implements_node).replace("implements", "").trim().to_string();
            signature.push_str(&format!(" implements {}", implements_clause));
        }

        let mut metadata = HashMap::new();
        metadata.insert("type".to_string(), serde_json::Value::String("enum".to_string()));
        if let Some(backing_type) = backing_type {
            metadata.insert("backingType".to_string(), serde_json::Value::String(backing_type));
        }
    ...
    fn extract_enum_case(&mut self, node: Node, parent_id: Option<&str>) -> Option<Symbol> {
        ...
        let mut metadata = HashMap::new();
        metadata.insert("type".to_string(), serde_json::Value::String("enum_case".to_string()));
        if let Some(val) = value {
    ...
    fn extract_variable_assignment(&mut self, node: Node, parent_id: Option<&str>) -> Option<Symbol> {
        ...
        let signature = format!("{} = {}", self.base.get_node_text(&variable_name_node), value_text);

        let mut metadata = HashMap::new();
        metadata.insert("type".to_string(), serde_json::Value::String("variable_assignment".to_string()));
    ...
    fn visit_relationships(&mut self, node: Node, symbols: &[Symbol], relationships: &mut Vec<Relationship>) {
        match node.kind() {
            "class_declaration" => {
    ...
    fn extract_class_relationships(&mut self, node: Node, symbols: &[Symbol], relationships: &mut Vec<Relationship>) {
        ...
        if let Some(extends_node) = self.find_child(&node, "base_clause") {
            let base_class_name = self.base.get_node_text(&extends_node).replace("extends", "").trim().to_string();
            ...
            if let Some(base_class_symbol) = symbols.iter().find(|s|
                ...
                relationships.push(Relationship {
                    ...
                    metadata: Some({
                        let mut metadata = HashMap::new();
                        metadata.insert("baseClass".to_string(), serde_json::Value::String(base_class_name));
                        metadata
                    }),
                });
            }
        ...
        if let Some(implements_node) = self.find_child(&node, "class_interface_clause") {
            let interface_names: Vec<String> = self.base.get_node_text(&implements_node)
                .replace("implements", "")
                .split(',')
            ...
            for interface_name in interface_names {
                ...
                relationships.push(Relationship {
                    ...
                    metadata: Some({
                        let mut metadata = HashMap::new();
                        metadata.insert("interface".to_string(), serde_json::Value::String(interface_name));
                        metadata
                    }),
                });
            }
    ...
    fn extract_interface_relationships(&mut self, node: Node, symbols: &[Symbol], relationships: &mut Vec<Relationship>) {
        ...
        if let Some(extends_node) = self.find_child(&node, "base_clause") {
            let base_interface_names: Vec<String> = self.base.get_node_text(&extends_node)
                .replace("extends", "")
                .split(',')
                .map(|name| name.trim().to_string())
                .collect();
            ...
            for base_interface_name in base_interface_names {
                relationships.push(Relationship {
                    ...
                    metadata: Some({
                        let mut metadata = HashMap::new();
                        metadata.insert("baseInterface".to_string(), serde_json::Value::String(base_interface_name));
                        metadata
                    }),
                });
            }
