...
/// Extract method
pub fn extract_method(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    ...
    let return_type = helpers::extract_return_type(base, &node).unwrap_or_else(|| "void".to_string());
    ...
    let type_param_str = type_params.as_ref().map(|tp| format!("{} ", tp)).unwrap_or_default();
...
/// Extract constructor
pub fn extract_constructor(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    let mut cursor = node.walk();
...
/// Extract destructor
pub fn extract_destructor(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    let mut cursor = node.walk();
    let name_node = node.children(&mut cursor).find(|c| c.kind() == "identifier")?;
...
/// Extract property
pub fn extract_property(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    let mut cursor = node.walk();
    let name_node = node.children(&mut cursor).find(|c| c.kind() == "identifier")?;
    let name = base.get_node_text(&name_node);
    let modifiers = helpers::extract_modifiers(base, &node);
    let visibility = helpers::determine_visibility(&modifiers, None);
    let prop_type = helpers::extract_property_type(base, &node).unwrap_or_else(|| "var".to_string());
...
/// Extract field
pub fn extract_field(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    let modifiers = helpers::extract_modifiers(base, &node);
    let visibility = helpers::determine_visibility(&modifiers, None);
...
/// Extract event
pub fn extract_event(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    let mut cursor = node.walk();
    let var_declaration = node.children(&mut cursor).find(|c| c.kind() == "variable_declaration")?;
    let mut var_cursor = var_declaration.walk();
    ...
    let type_node = var_declaration.children(&mut var_cursor).find(|c| c.kind() != "variable_declarator");
    let event_type = type_node.map(|node| base.get_node_text(&node)).unwrap_or_else(|| "EventHandler".to_string());
...
/// Extract delegate
pub fn extract_delegate(base: &mut BaseExtractor, node: Node, parent_id: Option<String>) -> Option<Symbol> {
    let mut cursor = node.walk();
    let children: Vec<Node> = node.children(&mut cursor).collect();
