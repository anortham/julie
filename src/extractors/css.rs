...
impl CSSExtractor {
        ...
        match node.kind() {
            ...
            "at_rule" | "import_statement" | "charset_statement" | "namespace_statement" => {
                if let Some(at_rule_symbol) =
                    self.extract_at_rule(node, current_parent_id.as_deref())
                {
            ...
            "keyframes_statement" => {
                if let Some(keyframes_symbol) =
                    self.extract_keyframes_rule(node, current_parent_id.as_deref())
                {
            ...
            "media_statement" => {
                if let Some(media_symbol) =
                    self.extract_media_rule(node, current_parent_id.as_deref())
                {
            ...
            "supports_statement" => {
                if let Some(supports_symbol) =
                    self.extract_supports_rule(node, current_parent_id.as_deref())
                {
            ...
            "property_name" => {
                ...
                if property_text.starts_with("--") {
                    if let Some(custom_prop) =
                        self.extract_custom_property(node, current_parent_id.as_deref())
                    {
    ...
    fn extract_key_properties(
        &self,
        declaration_block: &tree_sitter::Node,
        selector: Option<&str>,
    ) -> Vec<String> {
    ...
    fn find_property_value<'a>(
        &self,
        property_node: &tree_sitter::Node<'a>,
    ) -> Option<tree_sitter::Node<'a>> {
