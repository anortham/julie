...
pub(super) fn traverse_tree(
    ...
    match node.kind() {
        "function_definition_statement" | "function_declaration" => {
            symbol = functions::extract_function_definition_statement(symbols, base, node, parent_id.as_deref());
        }
        "local_function_definition_statement" | "local_function_declaration" => {
            symbol = functions::extract_local_function_definition_statement(symbols, base, node, parent_id.as_deref());
        }
        "local_variable_declaration" | "variable_declaration" => {
            symbol = variables::extract_local_variable_declaration(symbols, base, node, parent_id.as_deref());
        }
        "assignment_statement" => {
            symbol = variables::extract_assignment_statement(symbols, base, node, parent_id.as_deref());
        }
        "variable_assignment" => {
            symbol = variables::extract_variable_assignment(symbols, base, node, parent_id.as_deref());
        }
