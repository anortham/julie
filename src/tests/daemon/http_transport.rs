...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_http_julie_session_stays_active_until_delete_cleanup_finishes() {
        let fixture = RealServiceFixture::new();
        let workspace_id = fixture.workspace_id();
        let dependencies = Arc::clone(&fixture.dependencies);
        let server = HttpTransportServer::bind(
        ...
        let lock_conn = Connection::open(fixture.paths.daemon_db()).expect("open lock connection");
        lock_conn
            .execute_batch("BEGIN IMMEDIATE")
            .expect("hold daemon db write lock");
        ...
        let became_idle_before_cleanup = tokio::time::timeout(Duration::from_millis(500), async {
            loop {
                if fixture.sessions.active_count() == 0 {
                    return;
                }
        ...
        .is_ok();
        assert!(
            !became_idle_before_cleanup,
            "daemon drain must still see the session while DELETE cleanup is blocked"
        );

        lock_conn
            .execute_batch("COMMIT")
            .expect("release daemon db write lock");
        wait_for_session_count(&fixture.daemon_db, &workspace_id, 0).await;
        for _ in 0..100 {
            if fixture.sessions.active_count() == 0 {
                server.shutdown().await.unwrap();
        ...
        panic!(
            "DELETE cleanup should eventually remove daemon session tracker entry, active={}",
            fixture.sessions.active_count()
        );
    }
