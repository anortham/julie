...
"#;
        ...
        assert!(player.signature.contains("class_name Player"));
        ...
        assert!(take_damage.unwrap().signature.contains("func take_damage(amount: int) -> bool"));

        // Tool class
        let custom_resource = symbols.iter().find(|s| s.name == "CustomResource");
        assert!(custom_resource.is_some());
        assert!(custom_resource.unwrap().signature.contains("@tool"));
        ...
        assert!(health.signature.contains("var health: int = 100"));
        ...
        assert!(max_lives.signature.contains("const MAX_LIVES: int = 3"));
        ...
        assert!(instance_count.unwrap().signature.contains("static var instance_count"));

        // Setget properties
        let score = symbols.iter().find(|s| s.name == "_score");
        assert!(score.is_some());
        assert!(score.unwrap().signature.contains("setget set_score, get_score"));
        ...
        assert!(level.unwrap().signature.contains("var level: int = 1:"));
    }
...
"#;
        ...
        assert!(health_changed.signature.contains("signal health_changed(new_health: int)"));
        ...
        assert!(item_collected.unwrap().signature.contains("signal item_collected(item_name: String, quantity: int)"));
        ...
        assert!(ready.signature.contains("func _ready()"));

        let process = symbols.iter().find(|s| s.name == "_process");
        assert!(process.is_some());
        assert!(process.unwrap().signature.contains("func _process(delta: float)"));
        ...
        assert!(input.unwrap().signature.contains("func _input(event: InputEvent)"));
        ...
        assert!(function_with_params.unwrap().signature.contains("func function_with_params(name: String, age: int, active: bool = true)"));

        let function_with_return = symbols.iter().find(|s| s.name == "function_with_return");
        assert!(function_with_return.is_some());
        assert!(function_with_return.unwrap().signature.contains("-> Vector2"));

        // Static functions
        let calculate_distance = symbols.iter().find(|s| s.name == "calculate_distance");
        assert!(calculate_distance.is_some());
        assert!(calculate_distance.unwrap().signature.contains("static func calculate_distance"));

        // Virtual functions
        let can_drop_data = symbols.iter().find(|s| s.name == "_can_drop_data");
        assert!(can_drop_data.is_some());
        assert!(can_drop_data.unwrap().signature.contains("-> bool"));
        ...
        assert!(on_health_changed.unwrap().signature.contains("func _on_health_changed(new_health: int)"));
        ...
        assert!(fade_out.unwrap().signature.contains("func fade_out(duration: float = 1.0)"));
        ...
        assert!(process_data.signature.contains("Array[Dictionary]"));
        assert!(process_data.signature.contains("-> Array[String]"));
...
"#;
        ...
        assert!(player.signature.contains("@onready var player: CharacterBody2D = $Player"));
        ...
        assert!(camera.unwrap().signature.contains("$Player/Camera2D"));
        ...
        assert!(player_scene.signature.contains("preload(\"res://scenes/Player.tscn\")"));

        // Resource loading
        let enemy_texture = symbols.iter().find(|s| s.name == "enemy_texture");
        assert!(enemy_texture.is_some());
        assert!(enemy_texture.unwrap().signature.contains("load(\"res://textures/enemy.png\")"));
        ...
        assert!(handle_keyboard_input.unwrap().signature.contains("func _handle_keyboard_input(event: InputEventKey)"));
        ...
        assert!(spawn_enemy.unwrap().signature.contains("func spawn_enemy(position: Vector2)"));

        let find_nodes_by_group = symbols.iter().find(|s| s.name == "find_nodes_by_group");
        assert!(find_nodes_by_group.is_some());
        assert!(find_nodes_by_group.unwrap().signature.contains("-> Array[Node]"));
        ...
        assert!(load_game.unwrap().signature.contains("-> bool"));
...
"#;
        ...
        assert!(typed_array.signature.contains("var typed_array: Array[String]"));
        ...
        assert!(move_func.unwrap().signature.contains("func move(direction: Vector2)"));
        ...
        assert!(is_alive.unwrap().signature.contains("-> bool"));
        ...
        assert!(energy.unwrap().signature.contains("var energy: float:"));
        ...
        assert!(process_collection.unwrap().signature.contains("func process_collection[T]"));

        let safe_cast = symbols.iter().find(|s| s.name == "safe_cast");
        assert!(safe_cast.is_some());
        assert!(safe_cast.unwrap().signature.contains("func safe_cast[T]"));
        ...
        assert!(notification.unwrap().signature.contains("func _notification(what: int)"));
...
"#;
        ...
        assert!(version.unwrap().signature.contains("@export var version: String = \"1.0\""));
        ...
        assert!(serialize.signature.contains("-> Dictionary"));
        ...
        assert!(resource_loaded.signature.contains("signal resource_loaded(path: String, resource: Resource)"));
        ...
        assert!(load_resource_async.unwrap().signature.contains("-> Resource"));
        ...
        assert!(render_scale.unwrap().signature.contains("@export_range(0.5, 2.0)"));

        let quality_preset = symbols.iter().find(|s| s.name == "quality_preset");
        assert!(quality_preset.is_some());
        assert!(quality_preset.unwrap().signature.contains("@export_enum"));

        // Audio settings
        let master_volume = symbols.iter().find(|s| s.name == "master_volume");
        assert!(master_volume.is_some());
        assert!(master_volume.unwrap().signature.contains("@export_range(0.0, 1.0)"));
