...
#[tokio::test]
async fn test_parity_11_rename_symbol_preview() {
    let mut fixture = ProcessFixture::from_env().await;
    let ws = fixture.root().to_string_lossy().to_string();
    ...
    assert_tool_parity("rename_symbol", &cli_res, &mcp_res);
    fixture.shutdown().await;
}

#[tokio::test]
async fn test_parity_12_rewrite_symbol_preview() {
    let mut fixture = ProcessFixture::from_env().await;
    let ws = fixture.root().to_string_lossy().to_string();
        ...
        .rpc(json!({
            "jsonrpc": "2.0", "id": 12, "method": "tools/call", "params": {
                ...
                "arguments": {
                    ...
                    "content": "pub fn calculate_sum(a: i32, b: i32) -> i32 { a * b }",
                    "dry_run": true
                },
                "_meta": modern_meta()
            }
    ...
    assert_tool_parity("rewrite_symbol", &cli_res, &mcp_res);
    fixture.shutdown().await;
}
