...
/// Test: discover_indexable_files creates .julieignore even on clean projects
///
/// Regression test for the bug where .julieignore was only created when vendor
/// patterns were detected. Clean projects (no vendor dirs, or all vendor dirs
/// already covered by .gitignore) got no .julieignore at all.
///
/// Fix: c21a3a4e moved generate_julieignore_file() outside the
/// `if !detected_patterns.is_empty()` branch so it always runs.
#[test]
...
// ═══════════════════════════════════════════════════════════════════════
// Finding 1a: Daemon-state files added to BLACKLISTED_FILENAMES
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Finding 1b: Files under the configured JULIE_HOME are rejected by the
// discovery walker's `should_index_file` even if the home lives inside
// the workspace tree (an operator footgun we must defend against).
// ═══════════════════════════════════════════════════════════════════════
