...
"#;
        ...
        // Use statements
        let use_statement = symbols.iter().find(|s| s.name == "App\\Contracts\\UserRepositoryInterface");
        ...
        // Interface methods
        let draw = symbols.iter().find(|s| s.name == "draw" && s.parent_id.as_ref() == drawable.map(|d| &d.id));
        assert!(draw.is_some());
        assert!(draw.unwrap().signature.as_ref().unwrap().contains("public function draw(): void"));

        let get_color = symbols.iter().find(|s| s.name == "getColor" && s.parent_id.as_ref() == drawable.map(|d| &d.id));
        assert!(get_color.is_some());
        assert!(get_color.unwrap().signature.as_ref().unwrap().contains("getColor(): string"));

        let set_color = symbols.iter().find(|s| s.name == "setColor" && s.parent_id.as_ref() == drawable.map(|d| &d.id));
        assert!(set_color.is_some());
        assert!(set_color.unwrap().signature.as_ref().unwrap().contains("setColor(string $color): self"));

        // Abstract class
        let shape = symbols.iter().find(|s| s.name == "Shape");
        assert!(shape.is_some());
        assert!(shape.unwrap().signature.as_ref().unwrap().contains("abstract class Shape implements Drawable"));
        ...
        assert_eq!(color.unwrap().visibility.as_ref().unwrap(), &Visibility::Protected);
        assert!(color.unwrap().signature.as_ref().unwrap().contains("protected string $color = 'black'"));

        let instance_count = symbols.iter().find(|s| s.name == "instanceCount");
        assert!(instance_count.is_some());
        assert!(instance_count.unwrap().signature.as_ref().unwrap().contains("protected static int $instanceCount = 0"));
        ...
        assert_eq!(default_color.unwrap().visibility.as_ref().unwrap(), &Visibility::Public);
        assert!(default_color.unwrap().signature.as_ref().unwrap().contains("public const DEFAULT_COLOR = 'white'"));

        let max_size = symbols.iter().find(|s| s.name == "MAX_SIZE");
        assert!(max_size.is_some());
        assert_eq!(max_size.unwrap().visibility.as_ref().unwrap(), &Visibility::Private);

        // Constructor with parameters and default values
        let constructor = symbols.iter().find(|s| s.name == "__construct" && s.parent_id.as_ref() == shape.map(|sh| &sh.id));
        assert!(constructor.is_some());
        assert_eq!(constructor.unwrap().kind, SymbolKind::Constructor);
        assert!(constructor.unwrap().signature.as_ref().unwrap().contains("__construct(string $color = self::DEFAULT_COLOR)"));

        // Abstract method
        let get_area = symbols.iter().find(|s| s.name == "getArea" && s.parent_id.as_ref() == shape.map(|sh| &sh.id));
        assert!(get_area.is_some());
        assert!(get_area.unwrap().signature.as_ref().unwrap().contains("abstract public function getArea(): float"));

        // Static method
        let get_instance_count = symbols.iter().find(|s| s.name == "getInstanceCount");
        assert!(get_instance_count.is_some());
        assert!(get_instance_count.unwrap().signature.as_ref().unwrap().contains("public static function getInstanceCount(): int"));

        // Magic methods
        let to_string = symbols.iter().find(|s| s.name == "__toString");
        assert!(to_string.is_some());
        assert!(to_string.unwrap().signature.as_ref().unwrap().contains("public function __toString(): string"));
        ...
        assert!(circle.unwrap().signature.as_ref().unwrap().contains("class Circle extends Shape implements Serializable"));

        // Method with void return type
        let circle_draw = symbols.iter().find(|s| s.name == "draw" && s.parent_id.as_ref() == circle.map(|c| &c.id));
        assert!(circle_draw.is_some());
        assert!(circle_draw.unwrap().signature.as_ref().unwrap().contains("public function draw(): void"));
        ...
        assert!(get_dimensions.unwrap().signature.as_ref().unwrap().contains("final public function getDimensions(): array"));
    }
...
"#;
        ...
        assert!(status.unwrap().signature.as_ref().unwrap().contains("enum Status: string implements JsonSerializable"));

        // Enum cases
        let pending = symbols.iter().find(|s| s.name == "PENDING");
        assert!(pending.is_some());
        assert_eq!(pending.unwrap().kind, SymbolKind::EnumMember);
        assert!(pending.unwrap().signature.as_ref().unwrap().contains("case PENDING = 'pending'"));

        // Enum method
        let get_label = symbols.iter().find(|s| s.name == "getLabel" && s.parent_id.as_ref() == status.map(|st| &st.id));
        assert!(get_label.is_some());
        assert!(get_label.unwrap().signature.as_ref().unwrap().contains("public function getLabel(): string"));

        // Int enum
        let priority = symbols.iter().find(|s| s.name == "Priority");
        assert!(priority.is_some());
        assert!(priority.unwrap().signature.as_ref().unwrap().contains("enum Priority: int"));

        let low = symbols.iter().find(|s| s.name == "LOW");
        assert!(low.is_some());
        assert!(low.unwrap().signature.as_ref().unwrap().contains("case LOW = 1"));
        ...
        assert!(created_at.unwrap().signature.as_ref().unwrap().contains("protected ?\\DateTime $createdAt = null"));

        // Trait method
        let touch = symbols.iter().find(|s| s.name == "touch" && s.parent_id.as_ref() == timestampable.map(|t| &t.id));
        ...
        let cache = symbols.iter().find(|s| s.name == "cache" &&
            s.signature.as_ref().unwrap().contains("private static array"));
        assert!(cache.is_some());
        assert!(cache.unwrap().signature.as_ref().unwrap().contains("private static array $cache = []"));

        // Abstract method in trait
        let get_id = symbols.iter().find(|s| s.name == "getId" &&
            s.signature.as_ref().unwrap().contains("abstract"));
        assert!(get_id.is_some());
        assert!(get_id.unwrap().signature.as_ref().unwrap().contains("abstract public function getId(): int|string"));

        // Attribute class
        let api_resource = symbols.iter().find(|s| s.name == "ApiResource");
        assert!(api_resource.is_some());
        assert!(api_resource.unwrap().signature.as_ref().unwrap().contains("#[Attribute(Attribute::TARGET_CLASS | Attribute::TARGET_METHOD)]"));

        // Constructor property promotion
        let api_constructor = symbols.iter().find(|s| s.name == "__construct" && s.parent_id.as_ref() == api_resource.map(|ar| &ar.id));
        assert!(api_constructor.is_some());
        assert!(api_constructor.unwrap().signature.as_ref().unwrap().contains("public readonly string $version = 'v1'"));

        // Class with attributes
        let user = symbols.iter().find(|s| s.name == "User");
        assert!(user.is_some());
        assert!(user.unwrap().signature.as_ref().unwrap().contains("#[ApiResource(version: 'v2', scopes: ['read', 'write'])]"));
        assert!(user.unwrap().signature.as_ref().unwrap().contains("use Timestampable"));
        assert!(user.unwrap().signature.as_ref().unwrap().contains("use Cacheable"));

        // Property with attribute
        let name = symbols.iter().find(|s| s.name == "name" && s.parent_id.as_ref() == user.map(|u| &u.id));
        assert!(name.is_some());
        assert!(name.unwrap().signature.as_ref().unwrap().contains("#[Validate(rules: ['required', 'string', 'max:255'])]"));

        // Union type
        let trait_get_id = symbols.iter().find(|s| s.name == "getId" && s.parent_id.as_ref() == user.map(|u| &u.id));
        assert!(trait_get_id.is_some());
        assert!(trait_get_id.unwrap().signature.as_ref().unwrap().contains("getId(): int"));

        // Method with attribute
        let get_email = symbols.iter().find(|s| s.name == "getEmail");
        assert!(get_email.is_some());
        assert!(get_email.unwrap().signature.as_ref().unwrap().contains("#[ApiResource(deprecated: true)]"));

        // Readonly class
        let configuration = symbols.iter().find(|s| s.name == "Configuration");
        assert!(configuration.is_some());
        assert!(configuration.unwrap().signature.as_ref().unwrap().contains("readonly class Configuration"));

        // Constructor with property promotion
        let config_constructor = symbols.iter().find(|s| s.name == "__construct" && s.parent_id.as_ref() == configuration.map(|c| &c.id));
        assert!(config_constructor.is_some());
        assert!(config_constructor.unwrap().signature.as_ref().unwrap().contains("public string $database_url"));
        assert!(config_constructor.unwrap().signature.as_ref().unwrap().contains("public bool $debug_mode = false"));
    }
...
"#;
        ...
        assert!(calculate_sum.unwrap().signature.as_ref().unwrap().contains("function calculateSum(int $a, int $b): int"));

        // Function with callable parameter
        let process_data = symbols.iter().find(|s| s.name == "processData");
        assert!(process_data.is_some());
        assert!(process_data.unwrap().signature.as_ref().unwrap().contains("callable $callback = null"));
        ...
        assert!(create_multiplier.unwrap().signature.as_ref().unwrap().contains("createMultiplier(int $factor): \\Closure"));

        // Function returning reference
        let get_reference = symbols.iter().find(|s| s.name == "getReference");
        assert!(get_reference.is_some());
        assert!(get_reference.unwrap().signature.as_ref().unwrap().contains("function &getReference(array &$array, string $key): mixed"));

        // Function with variadic parameters
        let format_string = symbols.iter().find(|s| s.name == "formatString");
        assert!(format_string.is_some());
        assert!(format_string.unwrap().signature.as_ref().unwrap().contains("string ...$args"));

        // Function with union types
        let handle_value = symbols.iter().find(|s| s.name == "handleValue");
        assert!(handle_value.is_some());
        assert!(handle_value.unwrap().signature.as_ref().unwrap().contains("int|string|null $value"));

        // Function with complex parameters
        let process_user = symbols.iter().find(|s| s.name == "processUser");
        assert!(process_user.is_some());
        assert!(process_user.unwrap().signature.as_ref().unwrap().contains("?string $email = null"));
        assert!(process_user.unwrap().signature.as_ref().unwrap().contains("array $options = []"));

        // Variable assignments with closures
        let doubled = symbols.iter().find(|s| s.name == "doubled");
        assert!(doubled.is_some());
        assert!(doubled.unwrap().signature.as_ref().unwrap().contains("array_map(fn($n) => $n * 2, $numbers)"));

        let multiplier = symbols.iter().find(|s| s.name == "multiplier");
        assert!(multiplier.is_some());
        assert!(multiplier.unwrap().signature.as_ref().unwrap().contains("function(int $x, int $y): int"));

        let processor = symbols.iter().find(|s| s.name == "processor");
        assert!(processor.is_some());
        assert!(processor.unwrap().signature.as_ref().unwrap().contains("use ($multiplier)"));

        // Closure with reference capture
        let incrementer = symbols.iter().find(|s| s.name == "incrementer");
        assert!(incrementer.is_some());
        assert!(incrementer.unwrap().signature.as_ref().unwrap().contains("use (&$counter)"));

        // First-class callable syntax
        let strlen = symbols.iter().find(|s| s.name == "strlen" &&
            s.signature.as_ref().unwrap().contains("strlen(...)"));
        ...
        let add = symbols.iter().find(|s| s.name == "add" && s.parent_id.as_ref() == math_operations.map(|mo| &mo.id));
        assert!(add.is_some());
        assert!(add.unwrap().signature.as_ref().unwrap().contains("public static function add(int $a, int $b): int"));

        // Method references
        let add_ref = symbols.iter().find(|s| s.name == "add" &&
            s.signature.as_ref().unwrap().contains("MathOperations::add(...)"));
        assert!(add_ref.is_some());

        // Anonymous class
        let logger = symbols.iter().find(|s| s.name == "logger");
        assert!(logger.is_some());
        assert!(logger.unwrap().signature.as_ref().unwrap().contains("new class implements \\Psr\\Log\\LoggerInterface"));
    }
...
"#;
        ...
        let shape_drawable = relationships.iter().find(|r| {
            ...
            symbols.iter().find(|s| s.id == r.from_symbol_id).map(|s| &s.name) == Some(&"Shape".to_string()) &&
            symbols.iter().find(|s| s.id == r.to_symbol_id).map(|s| &s.name) == Some(&"Drawable".to_string())
        });
        ...
        let circle_shape = relationships.iter().find(|r| {
            ...
            symbols.iter().find(|s| s.id == r.to_symbol_id).map(|s| &s.name) == Some(&"Shape".to_string())
        });
        ...
        let circle_colorable = relationships.iter().find(|r| {
            r.kind == RelationshipKind::Implements &&
            symbols.iter().find(|s| s.id == r.from_symbol_id).map(|s| &s.name) == Some(&"Circle".to_string()) &&
            symbols.iter().find(|s| s.id == r.to_symbol_id).map(|s| &s.name) == Some(&"Colorable".to_string())
        });
