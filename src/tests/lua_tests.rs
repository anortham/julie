...
"#;
        ...
        assert!(calculate_area.unwrap().signature.contains("function calculateArea(width, height)"));
        assert_eq!(calculate_area.unwrap().visibility, "public");
        ...
        assert!(validate_input.unwrap().signature.contains("local function validateInput(value)"));
        assert_eq!(validate_input.unwrap().visibility, "private");
        ...
        assert!(multiply.unwrap().signature.contains("local multiply = function(a, b)"));

        // Short function
        let add = symbols.iter().find(|s| s.name == "add");
        assert!(add.is_some());
        assert!(add.unwrap().signature.contains("local add = function(x, y)"));

        // Global variables
        let pi = symbols.iter().find(|s| s.name == "PI");
        assert!(pi.is_some());
        assert_eq!(pi.unwrap().kind, SymbolKind::Variable);
        assert_eq!(pi.unwrap().visibility, "public");

        let version = symbols.iter().find(|s| s.name == "VERSION");
        assert!(version.is_some());
        assert!(version.unwrap().signature.contains("VERSION = \"1.0.0\""));
        ...
        assert!(user_name.unwrap().signature.contains("local userName = \"John Doe\""));

        let is_active = symbols.iter().find(|s| s.name == "isActive");
        assert!(is_active.is_some());
        assert!(is_active.unwrap().signature.contains("local isActive = true"));

        // Multiple assignment variables
        let x = symbols.iter().find(|s| s.name == "x");
        assert!(x.is_some());
        assert!(x.unwrap().signature.contains("local x, y, z = 10, 20, 30"));
        ...
        assert!(get_coordinates.unwrap().signature.contains("function getCoordinates()"));

        let sum_fn = symbols.iter().find(|s| s.name == "sum");
        assert!(sum_fn.is_some());
        assert!(sum_fn.unwrap().signature.contains("function sum(...)"));

        let greet = symbols.iter().find(|s| s.name == "greet");
        assert!(greet.is_some());
        assert!(greet.unwrap().signature.contains("function greet(name)"));
    }
...
"#;
        ...
        let multiply = symbols.iter().find(|s| s.name == "multiply" && s.signature.contains("calculator:multiply"));
        assert!(multiply.is_some());
        assert_eq!(multiply.unwrap().kind, SymbolKind::Method);

        let divide = symbols.iter().find(|s| s.name == "divide" && s.signature.contains("calculator:divide"));
        ...
        assert!(person.unwrap().signature.contains("function Person(name, age)"));
...
"#;
        ...
        // Require statements
        let json_require = symbols.iter().find(|s| s.name == "json" && s.signature.contains("require(\"json\")"));
        assert!(json_require.is_some());
        assert_eq!(json_require.unwrap().kind, SymbolKind::Import);

        let socket_require = symbols.iter().find(|s| s.name == "socket" && s.signature.contains("require(\"socket\")"));
        assert!(socket_require.is_some());

        let http_require = symbols.iter().find(|s| s.name == "http" && s.signature.contains("require(\"socket.http\")"));
        assert!(http_require.is_some());

        // Relative requires
        let utils_require = symbols.iter().find(|s| s.name == "utils" && s.signature.contains("require(\"./utils\")"));
        assert!(utils_require.is_some());

        let config_require = symbols.iter().find(|s| s.name == "config" && s.signature.contains("require(\"../config/settings\")"));
        ...
        assert!(load_module.unwrap().signature.contains("local function loadModule(name)"));

        let get_cached_module = symbols.iter().find(|s| s.name == "getCachedModule");
        assert!(get_cached_module.is_some());
        assert!(get_cached_module.unwrap().signature.contains("local function getCachedModule(name)"));
    }
...
"#;
        ...
        assert!(worker.unwrap().signature.contains("local function worker()"));
        ...
        assert!(async_operation.unwrap().signature.contains("local function asyncOperation(data, callback)"));
        ...
        assert!(async_fn.unwrap().signature.contains("local function async(fn)"));
...
"#;
        ...
        assert!(validate_email.unwrap().signature.contains("local function validateEmail(email)"));
