...
"#;
        ...
        assert_eq!(public_function.unwrap().visibility, Some(Visibility::Public));

        let private_function = symbols.iter().find(|s| s.name == "privateFunction");
        assert!(private_function.is_some());
        assert_eq!(private_function.unwrap().visibility, Some(Visibility::Private));
...
"#;
        ...
        let error_msg = symbols.iter().find(|s| s.name == "errorMsg" && s.visibility == Some(Visibility::Private));
        assert!(error_msg.is_some());
        assert_eq!(error_msg.unwrap().kind, SymbolKind::Variable);

        let status = symbols.iter().find(|s| s.name == "status" && s.visibility == Some(Visibility::Private));
