...
#[rustfmt::skip]
#[derive(Clone)]
pub struct AppState { pub engine: Arc<RequestEngine>, pub status: Arc<StatusLog>, pub token: Arc<str>, pub request_timeout: Duration, pub shutdown: tokio_util::sync::CancellationToken }
...
async fn shutdown(State(state): State<AppState>) -> Response {
    ...
    tokio::spawn(async move { tokio::time::sleep(Duration::from_millis(50)).await; token.cancel(); });
    (StatusCode::ACCEPTED, r#"{"stopping":true}"#).into_response()
}

async fn default_mcp_headers(request: axum::extract::Request, next: axum::middleware::Next) -> Response {
...
async fn api_call(State(state): State<AppState>, Path(tool): Path<String>, Json(arguments): Json<Map<String, Value>>) -> Response {
    let started = Instant::now();
    state.status.begin();
    let request = ToolRequest::new(tool.clone(), arguments);
    let context = RequestContext::new(RequestOrigin::Mcp, Some(state.request_timeout), tokio_util::sync::CancellationToken::new());
    let result = state.engine.execute(request, context).await;
    let latency_ms = started.elapsed().as_millis();
    match result {
        Ok(reply) => {
            state.status.end(RequestRecord { tool, workspace_id: reply.workspace_id.clone(), latency_ms, outcome: "ok", at: now_rfc3339() }, None);
            Json(reply).into_response()
        }
        Err(failure) => {
            state.status.end(RequestRecord { tool: tool.clone(), workspace_id: None, latency_ms, outcome: "error", at: now_rfc3339() },
...
pub fn failure_response(failure: RequestFailure) -> Response {
    let code = if failure.code == RequestFailure::internal("").code { StatusCode::INTERNAL_SERVER_ERROR } else { StatusCode::BAD_REQUEST };
