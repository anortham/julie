...
#[rustfmt::skip]
struct Inner { requests: VecDeque<RequestRecord>, errors: VecDeque<ErrorRecord>, last_activity: Instant, in_flight: usize }

#[rustfmt::skip]
#[derive(Serialize)]
pub struct StatusDocument {
    pub version: &'static str, pub pid: u32, pub uptime_seconds: u64,
    pub in_flight: usize, pub rss_bytes: Option<u64>,
    pub requests: Vec<RequestRecord>, pub errors: Vec<ErrorRecord>,
}

impl StatusLog {
    pub fn new() -> Self {
        let now = Instant::now();
        Self { started: now, inner: Mutex::new(Inner { requests: VecDeque::new(), errors: VecDeque::new(), last_activity: now, in_flight: 0 }) }
    }
    ...
    pub fn document(&self) -> StatusDocument {
        ...
        StatusDocument {
            ...
            uptime_seconds: self.started.elapsed().as_secs(), in_flight: inner.in_flight,
            rss_bytes: rss_bytes(), requests: inner.requests.iter().cloned().collect(),
...
fn rss_bytes() -> Option<u64> {
    ...
    statm.split_whitespace().nth(1)?.parse::<u64>().ok().map(|p| p * 4096)
}
