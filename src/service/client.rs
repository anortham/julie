...
impl std::fmt::Display for ConnectError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConnectError::VersionMismatch { service, client } => write!(f, "julie: service version {service} does not match client version {client}; run: julie-server service restart"),
...
pub fn exit_code(e: &ConnectError) -> i32 {
    match e { ConnectError::VersionMismatch { .. } => 3, ConnectError::Unavailable(_) => 1 }
}
...
async fn try_connect(paths: &RegistryPaths) -> Result<Option<ServiceClient>, ConnectError> {
    let Some(record) = discovery::read_record(paths).map_err(|e| ConnectError::Unavailable(e.to_string()))? else { return Ok(None); };
...
pub async fn connect_or_start(paths: &RegistryPaths, spawn: impl Fn() -> std::io::Result<()>) -> Result<ServiceClient, ConnectError> {
    if let Some(client) = try_connect(paths).await? { return Ok(client); }
...
pub fn spawn_detached_service() -> std::io::Result<()> {
    ...
    command.arg("service").stdin(std::process::Stdio::null()).stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null());
    #[cfg(unix)]
    { use std::os::unix::process::CommandExt; command.process_group(0); }
    #[cfg(windows)]
    { use std::os::windows::process::CommandExt; command.creation_flags(0x0000_0008 | 0x0000_0200); }
    command.spawn().map(|_| ())
}
