...
impl JulieServerHandler {
    #[tool(
        name = "fast_search",
        description = "Search code and symbols using unified code-aware full-text search. Supports multi-word queries with AND/OR logic, exact symbol name matches, file-path fragments, and conceptual semantic search. Optional `regions` restricts lexical line matches to stored comment, doc_comment/docstring, string_literal, or embedded spans. Optional `backend`: omitted backend runs hybrid symbol search for natural-language queries when embeddings are ready, else lexical mixed file+symbol hits (with labeled semantic fallback candidates on identifier-like zero-hit queries); explicit `lexical` stays pure lexical; `semantic` and `hybrid` are symbol-only concept search. Use lexical for file/path or region queries.",
        annotations(
