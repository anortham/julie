        ...
        match self.output_format.as_deref() {
            Some("toon") => {
                ...
                match toon_format::encode_default(&result) {
                    Ok(toon) => {
                        debug!("✅ Encoded fast_goto results to TOON ({} chars)", toon.len());
                        Ok(CallToolResult::text_content(vec![TextContent::from(toon)]))
                    }
                    Err(e) => {
                        warn!("❌ TOON encoding failed for fast_goto: {}, falling back to JSON", e);
            ...
            Some("auto") => {
                ...
                let total_results = result.definitions.len();
                if total_results >= 5 {
                    match toon_format::encode_default(&result) {
                        Ok(toon) => {
            ...
            _ => {
                ...
                debug!("✅ Returning fast_goto results as JSON-only (no redundant text_content)");
                Ok(CallToolResult::text_content(vec![])
                    .with_structured_content(structured_map))
            }
        }
