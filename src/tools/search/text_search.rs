...
pub(crate) fn definition_search_with_index_for_test(
    ...
    crate::search::scoring::apply_nl_path_prior(&mut symbol_results.results, query);
    symbol_results.results.truncate(limit);
    let mut symbols: Vec<crate::extractors::Symbol> = symbol_results
        .results
        .into_iter()
        .map(|h| {
            ...
            crate::extractors::Symbol {
                id: h.id,
                name: h.name,
                kind: crate::extractors::SymbolKind::try_from_string(&h.kind)
                    .unwrap_or(crate::extractors::SymbolKind::Variable),
                language: h.language,
...
pub(crate) async fn text_search_impl(
    query: &str,
