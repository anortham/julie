...
pub(crate) fn detect_language_for_indexing(path: &Path) -> String {
...
pub(crate) fn should_index_path_candidate(
    path: &Path,
    ...
    if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
        ...
        return crate::utils::file_utils::is_likely_text_file(path);
    }

    crate::utils::file_utils::is_likely_text_file(path)
}

pub(crate) fn should_watch_path(path: &Path, supported_extensions: &HashSet<String>) -> bool {
    should_index_path_candidate(path, supported_extensions)
}

pub(crate) fn should_process_deleted_path(
    path: &Path,
