...
pub fn workspace_root() -> PathBuf {
    resolve_workspace_root(env!("CARGO_MANIFEST_DIR"))
        .unwrap_or_else(|error| panic!("failed to resolve xtask-eval workspace root: {error}"))
}
...
fn resolve_workspace_root(manifest_dir: impl AsRef<Path>) -> Result<PathBuf> {
    ...
    let root = manifest_dir.parent().with_context(|| {
        format!(
            "expected xtask-eval manifest dir to have a parent: {}",
...
mod tests {
    ...
    fn workspace_root_points_to_repository_root() {
        ...
        assert!(root.join("xtask-eval").is_dir());
    }
    ...
    fn workspace_root_from_rejects_path_without_parent() {
        ...
        assert!(
            error
                .to_string()
                .contains("expected xtask-eval manifest dir to have a parent")
        );
    }
}
