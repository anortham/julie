...
fn record_rust_call_arg_literals(
    extractor: &mut RustExtractor,
    ...
    // Unwrap turbofish: `foo::<T>(args)` → inner callee for carrier derivation.
...
///
/// Plain `identifier` → its text (`load`). `field_expression`
/// (`client.get`, `conn.execute`) → the `value.field` join so bare DB verbs
/// (`execute`/`query`) match any receiver via the gate's last-segment rule.
/// `scoped_identifier` (`reqwest::get`, `sqlx::query`) → the immediate
/// `qualifier.name` join so dotted client APIs (`reqwest.get`) match config.
    ...
    match callee.kind() {
        ...
        "scoped_identifier" => {
            let name = callee
                ...
                .map(|n| base.get_node_text(&n));
            // Use the immediate qualifier (the last path segment before `name`)
            // so `reqwest::get` → `reqwest.get` and `std::fs::read` → `fs.read`.
...
///
/// Config-free: `carrier` is the macro name's last segment (`query`,
/// `query_as`, `query_scalar`) — no `!`. The shared sql carrier set matches
/// those by last-segment, so the macro literals flow through the same gate as
/// the call form (no extra TOML). `arg_position` is the string token's index
/// among the token-tree's literal/expression (named) tokens — 0 when the SQL is
/// the first macro arg (`query!`), later when a type precedes it (`query_as!`).
fn record_rust_macro_arg_literals(
    extractor: &mut RustExtractor,
    ...
    // The token_tree is the `(...)` / `{...}` / `[...]` argument group.
    let token_tree = {
    ...
    let containing_symbol_id = find_containing_symbol_id(macro_node, containing_symbols);
...
/// no `!`. `query!` → `query`; `sqlx::query!` → `query` (scoped name field);
/// `query_as!` → `query_as`. The sql carrier set matches these by last-segment.
