use crate::watcher::filtering::{
    build_gitignore_matcher, build_supported_extensions, contains_blacklisted_directory,
    is_gitignored, should_index_file, should_process_deletion,
};
use std::path::Path;
...
fn test_contains_blacklisted_directory() {
    ...
    assert!(contains_blacklisted_directory(Path::new(
        "/repo/.hg/store/data.i"
    )));
    assert!(contains_blacklisted_directory(Path::new(
        "/repo/.bzr/checkout/dirstate"
    )));
    assert!(contains_blacklisted_directory(Path::new(
        "/repo/.jj/repo/store/extra"
    )));
    assert!(contains_blacklisted_directory(Path::new(
        "/repo/_darcs/inventory"
    )));
