...
/// # Safety
///
/// Three checks prevent accidental deletion of production `.julie` directories:
/// 1. Refuses to delete from the crate manifest directory.
/// 2. Refuses to delete from the current working directory.
/// 3. Requires the path to be inside a recognised temp directory.
pub fn atomic_cleanup_julie_dir(workspace_path: &Path) -> Result<()> {
    ...
    // 🚨 SAFETY CHECK 1: NEVER delete .julie from the crate manifest directory during tests
    ...
    if workspace_canonical == manifest_canonical {
        panic!(
            "🚨 SAFETY VIOLATION: Attempted to delete .julie from the crate manifest directory!\n\
             Path: {}\n\
             Tests must NEVER delete data from source directories.\n\
    ...
    if !is_temp_dir {
        panic!(
            "🚨 SAFETY VIOLATION: Path is not in a recognised temp directory!\n\
             Path: {}\n\
    ...
    // All safety checks passed — attempt cleanup with exponential backoff.
