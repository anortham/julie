//! Dependency-direction tripwire for the `julie-pipeline` crate (Phase 2 PR 2a;
//! see `docs/plans/2026-06-04-julie-phase2-crate-split-design.md` and ADR-0006).
//!
//! `julie-pipeline` is the indexing + embedding layer. In the target DAG it sits
//! ABOVE `julie-core` and `julie-index` and BELOW `julie-tools` / `julie-runtime`
//! / the top `julie` crate; it is a SIBLING of `julie-context` (neither depends on
//! the other). It must therefore reach DOWN only into `julie-core` and
//! `julie-index` and must never reference any higher or sibling crate
//! (`julie-context`, `julie-tools`, `julie-runtime`, the parent `julie`).
//!
//! The Rust compiler already enforces this for real code references (pipeline has
...
/// julie-pipeline's executable source. Matched against comment-stripped lines.
///
/// Note: `julie_test_support` is intentionally NOT forbidden here (unlike the
/// julie-core tripwire). test-support depends only on julie-core, so a
/// julie-pipeline dev-dependency on it does not create a cycle.
...
/// Workspace path-dependencies julie-pipeline is allowed to declare. Everything
...
/// (not a workspace crate) and `julie-test-support` is a leaf test helper that
...
fn no_upward_source_references() {
    ...
    assert!(
        violations.is_empty(),
        "julie-pipeline sits below tools/runtime/julie and beside julie-context; \
...
fn manifest_has_no_cyclic_or_upward_dependency() {
    ...
    let content = fs::read_to_string(&manifest).expect("read julie-pipeline Cargo.toml");
    ...
    for (lineno, line) in content.lines().enumerate() {
        ...
        if trimmed.starts_with("julie ") || trimmed.starts_with("julie=") {
            violations.push(format!(
                "Cargo.toml:{}: julie-pipeline must NOT depend on the parent `julie` crate",
                lineno + 1
            ));
            continue;
        }
        // Any `julie-*` dependency outside the allowlist is a sibling or higher
        // crate (julie-context / julie-tools / julie-runtime, or a future crate
        // above pipeline). Forbidding by allowlist — rather than naming each
        ...
        if let Some(name) = workspace_dep_name(trimmed) {
            if name.starts_with("julie-") && !ALLOWED_WORKSPACE_DEPS.contains(&name) {
                violations.push(format!(
                    "Cargo.toml:{}: julie-pipeline must NOT depend on `{}` — only {:?} \
    ...
    assert!(
        violations.is_empty(),
        "Dependency-direction violation(s) in julie-pipeline/Cargo.toml:\n{}",
        violations.join("\n")
    );
}
