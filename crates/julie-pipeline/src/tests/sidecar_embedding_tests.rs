...
mod tests {
    ...
    // test_sidecar_root_path_succeeds_from_source_checkout lives in the top-crate test
    // tree (src/tests/core/sidecar_embedding_tests.rs) because it relies on
    // CARGO_MANIFEST_DIR pointing to the workspace root (which has python/embeddings_sidecar).
    // From crates/julie-pipeline, CARGO_MANIFEST_DIR is the pipeline crate, not the root.
}
