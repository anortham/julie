...
///
/// Exposed as `pub` (not `#[cfg(test)]`) so it can be called from test code in
/// the top-level `julie` crate via the `julie_pipeline::resolver` re-export.
/// The production code path never calls this function.
