...
use julie_index::search::tokenizer::{CodeTokenizer, split_camel_case};
...
pub fn looks_like_file_or_path_query(query: &str) -> bool {
    query
        .split_whitespace()
        .any(|token| token.contains('/') || token.contains('\\') || has_known_file_extension(token))
}

pub fn looks_like_identifier_probe_query(query: &str) -> bool {
...
pub fn clean_or_disjunction_terms(query: &str) -> Option<Vec<String>> {
    let trimmed = query.trim();
    if !trimmed.contains(" OR ") {
        return None;
    }
...
pub fn tokenize_text_for_line_match(text: &str) -> HashSet<String> {
...
pub fn term_matches_line(term: &str, line: &str, line_tokens: &HashSet<String>) -> bool {
