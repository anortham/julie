...
pub struct LineModeSearchResult {
...
struct LineModeCandidateWindow {
    file_results: Vec<julie_index::search::index::UnifiedHit>,
    saturated: bool,
}
...
pub struct LineModeStageCounts {
...
where
    S: FnMut(usize) -> Result<LineModeCandidateWindow>,
    C: FnMut(
        &[julie_index::search::index::UnifiedHit],
    ) -> Result<(Vec<LineMatch>, LineModeStageCounts, bool)>,
{
...
pub fn attribute_zero_hit_reason(counts: &LineModeStageCounts) -> Option<ZeroHitReason> {
...
pub async fn line_mode_matches(
    query: &str,
    language: &Option<String>,
    file_pattern: &Option<String>,
    limit: u32,
