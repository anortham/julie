...
impl SearchExecutionWorkspace {
    ...
    pub fn target(workspace_id: String) -> Self {
        Self {
            target: WorkspaceTarget::Target(workspace_id.clone()),
            workspace_id,
        }
    }
}

/// Inner helper result: run the unified search over one snapshot with a single
/// `file_pattern` value, apply test-exclude filtering, and return both the hits
/// and the pre-filter counts the caller needs to attribute zero-hit runs.
pub(crate) struct UnifiedPassResult {
    pub hits: Vec<SearchHit>,
    pub relaxed: bool,
    pub total_results: usize,
    /// Total raw-hit count *before* the file_pattern filter. Used to attribute
    /// FilePatternFiltered when this is non-zero but `pre_test_filter_total`
    /// is zero.
