...
/// `use julie_core::database::SymbolDatabase` caused it to rank above the actual
...
fn titles_for_files_excludes_import_kind_symbols() -> Result<()> {
    ...
    db.store_file_info(&julie_core::database::FileInfo {
        ...
        symbol_count: 1,
        line_count: 30,
        content: Some("use julie_core::database::SymbolDatabase;".to_string()),
    })
    .unwrap();
    db.store_symbols(&[Symbol {
        ...
        semantic_group: None,
        confidence: None,
        code_context: Some("use julie_core::database::SymbolDatabase;".to_string()),
        content_type: None,
        body_span: None,
        body_hash: None,
        annotations: Vec::new(),
