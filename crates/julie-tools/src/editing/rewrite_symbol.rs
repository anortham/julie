...
#[derive(Debug)]
pub struct RewriteSymbolFailure {
    pub kind: &'static str,
    pub message: String,
}
...
fn detect_language_name(file_path: &str) -> String {
    julie_core::language::detect_language(std::path::Path::new(file_path))
        .map(|l| format!("{l:?}"))
...
impl RewriteSymbolTool {
    ...
    pub fn call_prepared(&self, application: PreparedRewrite) -> Result<CallToolResult> {
